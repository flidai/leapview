...
// NewLegacyProvenance retains version-four provenance while completing an
// interrupted candidate created before target authorization policy revisions
// existed. The legacy validator rejects any revision/fingerprint fields that
// would make the record look like current provenance.
func NewLegacyProvenance(input ProvenanceInput) (Provenance, error) {
	return newProvenanceVersion(input, legacyProvenanceVersion)
}
...
func provenanceInvalid(err error) error { return fmt.Errorf("%w: %w", ErrProvenanceInvalid, err) }
