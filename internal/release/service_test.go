...
func (r *serviceTestReleaseRepository) RecordArtifact(ctx context.Context, artifact Artifact) error {
	r.recorded = true
	if intent, ok := AuditIntentFromContext(ctx); ok {
		r.recordedAuditMetadata = intent.MetadataJSON
	}
