...
type CandidateArtifactRecoveryRequest struct {
	...
	// AuthorizationFingerprint is the immutable compiled policy digest retained
	// by both legacy and current plans/seals. Legacy requests have no target
	// policy revision, so this remains their authoritative authorization fence.
