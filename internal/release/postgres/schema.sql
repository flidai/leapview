...
-- Durable release-transition operation identity, phase results, and one
-- transition-wide lease fence.  The migration copy is the production schema
-- source; this embedded copy keeps sqlc and isolated repository tests exact.
...
BEGIN
    ...
    IF NEW.operation_id IS DISTINCT FROM OLD.operation_id OR NEW.phase IS DISTINCT FROM OLD.phase
       ...
       OR (OLD.result_status = 'succeeded' AND NEW.result_status <> OLD.result_status)
       OR (OLD.result_status = 'failed' AND NEW.result_status <> OLD.result_status)
       OR (OLD.result_status = 'indeterminate' AND NEW.result_status <> OLD.result_status) THEN
        RAISE EXCEPTION 'release transition phase result is immutable';
    END IF;
    RETURN NEW;
END;
