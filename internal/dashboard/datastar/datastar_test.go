...
func testVisualDefinition(t *testing.T, id string) visualizationdefinition.Definition {
	...
	definitions, err := dashboardcompiler.CompileVisualizationDefinitions(&dashboardauthoring.Dashboard{
		...
		Visuals: dashboardauthoring.ChartVisualizations(map[string]dashboardauthoring.Visual{id: {Type: "bar", Title: id, Query: dashboardauthoring.VisualQuery{Table: "table", Metrics: []dashboardauthoring.FieldRef{{Field: "metric"}}}}}),
	})
	if err != nil {
		t.Fatal(err)
	}
