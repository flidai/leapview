...
INSERT INTO dashboard_view_days
  (workspace_id, dashboard_id, principal_id, viewed_on, page_id, first_viewed_at, last_viewed_at)
VALUES
  (sqlc.arg(workspace_id), sqlc.arg(dashboard_id), sqlc.arg(principal_id), sqlc.arg(viewed_on),
   sqlc.arg(page_id), sqlc.arg(viewed_at), sqlc.arg(viewed_at))
ON CONFLICT(workspace_id, dashboard_id, principal_id, viewed_on) DO UPDATE SET
...
ORDER BY viewer_count DESC, viewer_days DESC, last_viewed_at DESC, workspace_id, dashboard_id;
