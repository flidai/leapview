...
type restoredDataRuntime struct {
	rows       []dataquery.Row
	failTarget string
...
type restoredVerifyingRuntime struct {
	restoredDataRuntime
	verifyErr error
	verifies  int
}

func (r *restoredVerifyingRuntime) VerifySemantic(context.Context) error {
	r.verifies++
	return r.verifyErr
}
func (r *restoredDataRuntime) reportRows() report.QueryRows {
...
func (r *restoredDataRuntime) Query(context.Context, report.AggregateQuery) (report.QueryRows, error) {
	return r.reportRows(), nil
}
func (r *restoredDataRuntime) Rows(context.Context, report.RowQuery) (report.QueryRows, error) {
	return r.reportRows(), nil
}
func (r *restoredDataRuntime) Count(context.Context, report.CountQuery) (int, error) {
	return len(r.rows), nil
}
func (r *restoredDataRuntime) Histogram(context.Context, report.RawValueQuery, int) ([]report.HistogramBin, error) {
	return nil, nil
}
func (r *restoredDataRuntime) Distribution(context.Context, report.RawValueQuery, []report.QuerySort, int) (report.QueryRows, error) {
	return r.reportRows(), nil
}
func (r *restoredDataRuntime) ExecuteDataQuery(_ context.Context, query dataquery.Query) (dataquery.Result, error) {
	if r.queries != nil {
...
func (r *restoredDataRuntime) Refresh(context.Context) error       { return nil }
func (r *restoredDataRuntime) Close() error                        { return nil }
func (r *restoredDataRuntime) LastRefresh() time.Time              { return time.Now() }
func (r *restoredDataRuntime) setPlanner(p *semanticquery.Planner) { r.planner = p }
func (r *restoredDataRuntime) Planner() consumer.Planner           { return r.planner }

type restoredFactory struct {
	runtime DataRuntime
	err     error
}

func (f restoredFactory) OpenDashboardProjectDataRuntimes(_ context.Context, config ProjectDataRuntimeConfig) (map[graph.ResourceID]DataRuntime, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func restoredBase(kind, title string, fields []visualizationir.VisualizationField) visualizationir.VisualizationSpecBase {
...
func restoredCartesian(t *testing.T, id string) visualizationdefinition.Definition {
	...
	base := restoredBase("cartesian", id, fields)
...
func restoredTable(t *testing.T) visualizationdefinition.Definition {
	...
	base := restoredBase("table", "Orders", fields)
...
func restoredBehaviorDefinition(t *testing.T, withTable bool) (*ProjectDefinition, dashboarddefinition.Definition) {
	...
	good := restoredCartesian(t, "good")
	broken := restoredCartesian(t, "broken")
...
func restoredBehaviorRuntime(t *testing.T, definition *ProjectDefinition, data DataRuntime, factoryErr error) *Service {
	identity, err := graph.NewServingIdentity("project_1", "test", "generation_1")
	if err != nil {
		t.Fatal(err)
	}
	service, err := NewFromGeneration(context.Background(), t.TempDir(), restoredFactory{runtime: data, err: factoryErr}, identity, definition)
	if err != nil {
		t.Fatal(err)
	}
	return service
...
func TestRestoredMissingDataReturnsSetupPatch(t *testing.T) {
	definition, _ := restoredBehaviorDefinition(t, false)
	service := restoredBehaviorRuntime(t, definition, nil, &materializeruntime.MissingDataError{Missing: []string{"orders.csv"}})
	defer service.Close()
...
func TestRestoredServiceVerifyRunsGovernedSemanticVerifier(t *testing.T) {
	definition, _ := restoredBehaviorDefinition(t, false)
	data := &restoredVerifyingRuntime{restoredDataRuntime: restoredDataRuntime{rows: []dataquery.Row{{"status": "A", "value": int64(1)}}}}
	service := restoredBehaviorRuntime(t, definition, data, nil)
	defer service.Close()
...
func TestRestoredServiceVerifyFailsClosedOnSemanticVerifierError(t *testing.T) {
	definition, _ := restoredBehaviorDefinition(t, false)
	data := &restoredVerifyingRuntime{restoredDataRuntime: restoredDataRuntime{rows: []dataquery.Row{{"status": "A", "value": int64(1)}}}, verifyErr: errors.New("representative plan failed")}
	service := restoredBehaviorRuntime(t, definition, data, nil)
	defer service.Close()
...
func TestRestoredPageQueryIsolatesVisualizationFailure(t *testing.T) {
	definition, compiled := restoredBehaviorDefinition(t, false)
	...
	service := restoredBehaviorRuntime(t, definition, &restoredDataRuntime{rows: []dataquery.Row{{"status": "A", "value": int64(1)}}, failTarget: "missing_table"}, nil)
	defer service.Close()
	patch, err := service.QueryDashboardPage(context.Background(), "dashboard_1", "overview", dashboard.Filters{})
...
func TestRestoredQueriesFlowThroughAuditedDataQueryBoundary(t *testing.T) {
	definition, _ := restoredBehaviorDefinition(t, false)
	var queries []dataquery.Query
	service := restoredBehaviorRuntime(t, definition, &restoredDataRuntime{rows: []dataquery.Row{{"status": "A", "value": int64(1)}}, queries: &queries}, nil)
	defer service.Close()
...
func TestRestoredTableRowsRespectInteractiveCap(t *testing.T) {
	definition, _ := restoredBehaviorDefinition(t, true)
	...
	service := restoredBehaviorRuntime(t, definition, &restoredDataRuntime{rows: rows}, nil)
	defer service.Close()
...
func TestRestoredPowerFiltersTranslateComparisonAndRangePredicates(t *testing.T) {
