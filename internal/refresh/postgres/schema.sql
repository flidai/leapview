...
-- A manual request can outlive the run that currently owns the target. Keep
-- accepted intent separately from immutable execution history, reserving the
-- run identity before a worker admits the request.
...
-- The serving runtime currently shares one DuckLake publication target per
-- project/environment. Retain target_id per request for future partitioning,
-- while admission and the single active claim are fenced at that runtime scope.
...
BEGIN
    ...
    RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS manual_intent_insert_guard ON refresh.manual_intent;
...
BEGIN
    ...
    RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS manual_intent_guard ON refresh.manual_intent;
...
-- Run inserts and waiting-intent claims share this scope lock. A non-intent
-- root cannot slip in between the idle-target check and dispatch of the
-- reserved run; the matching reserved ID is the sole allowed claimant.
...
BEGIN
    ...
    RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS manual_intent_notify_change ON refresh.manual_intent;
...
LANGUAGE plpgsql SET search_path = pg_catalog, refresh AS $$
BEGIN
    IF NEW.schedule_revision_id IS DISTINCT FROM OLD.schedule_revision_id OR NEW.schedule_id IS DISTINCT FROM OLD.schedule_id OR
       NEW.scenario IS DISTINCT FROM OLD.scenario OR NEW.operation IS DISTINCT FROM OLD.operation OR
       NEW.policy_version IS DISTINCT FROM OLD.policy_version OR NEW.policy_sha256 IS DISTINCT FROM OLD.policy_sha256 OR
       NEW.target_scope IS DISTINCT FROM OLD.target_scope OR NEW.artifact_identity IS DISTINCT FROM OLD.artifact_identity OR
       NEW.cron IS DISTINCT FROM OLD.cron OR NEW.timezone IS DISTINCT FROM OLD.timezone OR
       NEW.stale_after IS DISTINCT FROM OLD.stale_after OR NEW.valid_from IS DISTINCT FROM OLD.valid_from THEN
        RAISE EXCEPTION 'recovery qualification schedule identity is immutable';
    END IF;
...
LANGUAGE plpgsql SET search_path = pg_catalog, refresh AS $$
BEGIN
    IF NEW.occurrence_id IS DISTINCT FROM OLD.occurrence_id OR NEW.request_digest IS DISTINCT FROM OLD.request_digest OR
       NEW.schedule_id IS DISTINCT FROM OLD.schedule_id OR NEW.schedule_revision_id IS DISTINCT FROM OLD.schedule_revision_id OR
       NEW.scenario IS DISTINCT FROM OLD.scenario OR NEW.operation IS DISTINCT FROM OLD.operation OR
       NEW.policy_version IS DISTINCT FROM OLD.policy_version OR NEW.policy_sha256 IS DISTINCT FROM OLD.policy_sha256 OR
       NEW.target_scope IS DISTINCT FROM OLD.target_scope OR NEW.artifact_identity IS DISTINCT FROM OLD.artifact_identity OR
       NEW.planned_at IS DISTINCT FROM OLD.planned_at OR NEW.expires_at IS DISTINCT FROM OLD.expires_at OR NEW.created_at IS DISTINCT FROM OLD.created_at THEN
        RAISE EXCEPTION 'recovery qualification occurrence identity is immutable';
    END IF;
...
LANGUAGE plpgsql SET search_path = pg_catalog, refresh AS $$
BEGIN
    IF NEW.status='claimed' AND OLD.status='pending' AND NOT EXISTS (
        SELECT 1 FROM refresh.recovery_qualification_attempt a WHERE a.occurrence_id=NEW.occurrence_id AND a.attempt_number=NEW.attempt_count AND a.fence_generation=NEW.fence_generation AND a.status IN ('claimed','running')) THEN
        RAISE EXCEPTION 'recovery qualification claim requires durable attempt evidence';
    END IF;
    IF NEW.status IN ('succeeded','failed','canceled') AND NEW.status IS DISTINCT FROM OLD.status AND NOT EXISTS (
        SELECT 1 FROM refresh.recovery_qualification_attempt a WHERE a.occurrence_id=NEW.occurrence_id AND a.attempt_number=NEW.attempt_count AND a.fence_generation=NEW.fence_generation AND a.status=NEW.status) THEN
        RAISE EXCEPTION 'terminal recovery qualification requires matching attempt evidence';
    END IF;
    IF NEW.status='pending' AND OLD.status IN ('claimed','running') AND NOT EXISTS (
        SELECT 1 FROM refresh.recovery_qualification_attempt a WHERE a.occurrence_id=NEW.occurrence_id AND a.attempt_number=NEW.attempt_count AND a.fence_generation=NEW.fence_generation AND a.status='abandoned') THEN
        RAISE EXCEPTION 'requeued recovery qualification requires abandoned attempt evidence';
    END IF;
    IF NEW.evidence_status='claimed' AND OLD.evidence_status IN ('pending','failed') AND NOT EXISTS (
        SELECT 1 FROM refresh.recovery_qualification_evidence_attempt a WHERE a.occurrence_id=NEW.occurrence_id AND a.attempt_number=NEW.evidence_attempt_count AND a.fence_generation=NEW.evidence_fence_generation AND a.status='claimed') THEN
        RAISE EXCEPTION 'recovery evidence claim requires durable attempt evidence';
    END IF;
    IF NEW.evidence_status IN ('published','failed') AND OLD.evidence_status='claimed' AND NOT EXISTS (
        SELECT 1 FROM refresh.recovery_qualification_evidence_attempt a WHERE a.occurrence_id=NEW.occurrence_id AND a.attempt_number=NEW.evidence_attempt_count AND a.fence_generation=NEW.evidence_fence_generation AND ((NEW.evidence_status='published' AND a.status='published') OR (NEW.evidence_status='failed' AND a.status IN ('failed','abandoned')))) THEN
        RAISE EXCEPTION 'terminal recovery evidence state requires matching attempt evidence';
    END IF;
    RETURN NEW;
END; $$;
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='leapview_control_runtime') THEN
        ...
        GRANT SELECT, INSERT, UPDATE ON refresh.schedule_revision, refresh.run, refresh.manual_intent, refresh.schedule_occurrence, refresh.attempt, refresh.publication_link, refresh.recovery_state, refresh.data_version, refresh.recovery_qualification_schedule, refresh.recovery_qualification_enqueue_cursor, refresh.recovery_qualification_occurrence, refresh.recovery_qualification_attempt, refresh.recovery_qualification_evidence_attempt TO leapview_control_runtime;
        GRANT EXECUTE ON FUNCTION refresh.fail_child_runs(text,text), refresh.complete_child_runs(text), refresh.close_omitted_schedules(text,text,text,text[],text[]), refresh.retain_recovery_qualification_occurrences(timestamptz,timestamptz,integer) TO leapview_control_runtime;
        REVOKE DELETE ON refresh.schedule_revision, refresh.run, refresh.manual_intent, refresh.schedule_occurrence, refresh.attempt, refresh.publication_link, refresh.recovery_state, refresh.data_version, refresh.recovery_qualification_schedule, refresh.recovery_qualification_enqueue_cursor, refresh.recovery_qualification_occurrence, refresh.recovery_qualification_attempt, refresh.recovery_qualification_evidence_attempt FROM leapview_control_runtime;
