...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='leapview_control_runtime') THEN
        ...
        GRANT SELECT, INSERT, UPDATE ON refresh.schedule_revision, refresh.run, refresh.schedule_occurrence, refresh.attempt, refresh.publication_link, refresh.recovery_state, refresh.data_version, refresh.recovery_qualification_schedule, refresh.recovery_qualification_occurrence, refresh.recovery_qualification_attempt, refresh.recovery_qualification_evidence_attempt TO leapview_control_runtime;
        GRANT EXECUTE ON FUNCTION refresh.fail_child_runs(text,text), refresh.complete_child_runs(text), refresh.close_omitted_schedules(text,text,text,text[],text[]), refresh.retain_recovery_qualification_occurrence(text,timestamptz,timestamptz) TO leapview_control_runtime;
