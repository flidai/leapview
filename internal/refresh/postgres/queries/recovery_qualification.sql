...
-- name: AdvanceRecoveryQualificationSchedule :exec
UPDATE refresh.recovery_qualification_schedule SET next_run_at=$1,updated_at=$2 WHERE schedule_revision_id=$3;
-- name: ListDueRecoveryQualificationScheduleIDs :many
SELECT schedule_revision_id FROM refresh.recovery_qualification_schedule WHERE enabled AND closed_at IS NULL AND next_run_at<=$1 ORDER BY next_run_at,schedule_id FOR UPDATE;
...
-- name: ListRecoveryQualificationOccurrenceIDsCreated :many
SELECT occurrence_id FROM refresh.recovery_qualification_occurrence WHERE schedule_revision_id=$1 AND created_at=$2 ORDER BY planned_at,occurrence_id;
...
UPDATE refresh.recovery_qualification_occurrence SET status='running',started_at=$4 WHERE occurrence_id=$1 AND lease_owner=$2 AND fence_generation=$3 AND lease_expires_at>$4 AND status='claimed';
-- name: StartRecoveryQualificationAttempt :execrows
UPDATE refresh.recovery_qualification_attempt SET status='running',started_at=$3 WHERE occurrence_id=$1 AND fence_generation=$2 AND status='claimed';
...
UPDATE refresh.recovery_qualification_occurrence SET restore_started_at=$4 WHERE occurrence_id=$1 AND lease_owner=$2 AND fence_generation=$3 AND lease_expires_at>$4 AND status='running' AND operation='restore' AND restore_started_at IS NULL AND restore_completed_at IS NULL;
...
UPDATE refresh.recovery_qualification_occurrence SET readiness_started_at=$4 WHERE occurrence_id=$1 AND lease_owner=$2 AND fence_generation=$3 AND lease_expires_at>$4 AND status='running' AND operation IN ('upgrade','rollback') AND readiness_started_at IS NULL AND readiness_completed_at IS NULL;
...
-- name: RetainRecoveryQualificationOccurrence :one
SELECT refresh.retain_recovery_qualification_occurrence($1,$2,$3);
