...
func TestSemanticDefinitionsValidateTypedMetricsDimensionsAndMetrics(t *testing.T) {
	tests := []struct {
		...
		{
			name: "count input conflict",
			mutate: func(model *Model) {
				model.Metrics["rating_count"] = Metric{Type: "aggregate", Dataset: "ratings", Aggregation: "count", Input: &MetricInput{Field: "ratings.score", Expression: "1"}, Empty: "zero"}
			},
			wantErr: "count requires at most one input",
		},
		{
		...
		},
		{
			...
			mutate: func(model *Model) {
				model.Metrics["rating_total"] = Metric{Type: "aggregate", Dataset: "ratings", Aggregation: "sum", Input: &MetricInput{Expression: "safe_divide(${ratings.score}, 2)"}, Empty: "null"}
			},
			wantErr: "aggregate input is required",
		},
		{
			...
			mutate: func(model *Model) {
				model.Dimensions["activity_date"] = SemanticDimension{Type: "timestamp", Grains: []string{"fortnight"}, Bindings: map[string]DimensionBinding{"ratings": {Field: "ratings.rated_at"}}}
			},
...
func TestSemanticTimeGrainRetainsNativeOrderAndDatasetMetricBindings(t *testing.T) {
	...
	model.Dimensions["activity_date"] = SemanticDimension{
		Type: "timestamp", NativeGrain: "month", Grains: []string{"month", "quarter", "year"},
	...
	model.Dimensions["activity_date"] = SemanticDimension{
		Type: "timestamp", NativeGrain: "month", Grains: []string{"day", "month"},
		Bindings: map[string]DimensionBinding{"ratings": {Field: "ratings.rated_at"}},
	}
	...
	model.Dimensions["activity_date"] = SemanticDimension{Type: "timestamp", Grains: []string{"month"}, Bindings: map[string]DimensionBinding{"ratings": {Field: "ratings.rated_at"}}}
	...
	model.Dimensions["activity_date_override"] = SemanticDimension{Type: "string", Bindings: map[string]DimensionBinding{"ratings": {Field: "ratings.movie_id"}}}
