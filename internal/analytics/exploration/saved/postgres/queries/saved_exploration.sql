-- Native PostgreSQL sqlc leaves for the saved-exploration authority.
-- Lifecycle queries intentionally never select the authored payload bytes.

-- name: AcquireSavedExplorationRetryLock :exec
...
JOIN saved_exploration.saved_exploration_revisions r
 ...
 AND r.content_hash = e.current_content_hash
WHERE e.project_id = sqlc.arg(project_id)
  AND (sqlc.arg(include_archived) OR e.status = 'active')
...
-- name: HasSavedExplorationIdentity :one
SELECT EXISTS (
    SELECT 1 FROM saved_exploration.saved_explorations
    WHERE project_id = sqlc.arg(project_id) AND exploration_id = sqlc.arg(exploration_id)
);

-- name: InsertSavedExploration :execrows
...
VALUES
 (sqlc.arg(project_id), sqlc.arg(exploration_id), sqlc.arg(owner_principal_id),
  sqlc.arg(title), sqlc.arg(slug), sqlc.arg(visibility), sqlc.arg(status),
  sqlc.arg(semantic_model_id), sqlc.arg(created_at), sqlc.arg(updated_at), NULL,
  sqlc.arg(current_revision_id), sqlc.arg(current_revision_number), sqlc.arg(current_content_hash))
...
VALUES
 (sqlc.arg(project_id), sqlc.arg(exploration_id), sqlc.arg(revision_id),
  sqlc.arg(revision_number), sqlc.arg(spec_envelope_version), sqlc.arg(spec_canonical_json),
  sqlc.arg(content_hash), sqlc.arg(created_by), sqlc.arg(created_at),
  sqlc.arg(serving_project_id), sqlc.arg(serving_environment), sqlc.arg(serving_generation_id));

-- name: InsertSavedExplorationOperation :execrows
...
VALUES
 (sqlc.arg(project_id), sqlc.arg(actor_id), sqlc.arg(operation_kind),
  ...
  sqlc.arg(result_created_at), sqlc.arg(result_updated_at), sqlc.arg(result_archived_at),
...
-- name: UpdateSavedExplorationVersion :execrows
UPDATE saved_exploration.saved_explorations
SET title = sqlc.arg(title), slug = sqlc.arg(slug), visibility = sqlc.arg(visibility),
    semantic_model_id = sqlc.arg(semantic_model_id), updated_at = sqlc.arg(updated_at),
    current_revision_id = sqlc.arg(revision_id), current_revision_number = sqlc.arg(revision_number),
...
-- name: ArchiveSavedExploration :execrows
UPDATE saved_exploration.saved_explorations
SET status = 'archived', archived_at = sqlc.arg(archived_at), updated_at = sqlc.arg(archived_at)
WHERE project_id = sqlc.arg(project_id) AND exploration_id = sqlc.arg(exploration_id)
  AND status = 'active'
  AND current_revision_id = sqlc.arg(expected_revision_id)
  AND current_revision_number = sqlc.arg(expected_revision_number)
