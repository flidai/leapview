...
func (p *testProvider) Acquire(context.Context) (projectruntime.Lease, error) {
	...
	if p.lease == nil {
		...
		p.lease = &testLease{runtime: testRuntime{model: testModel(), projections: &p.projections}, identity: identity}
	}
	return p.lease, nil
}
...
func TestReadRejectsInvalidLeaseIdentityBeforeRepositoryAccess(t *testing.T) {
	repo := seededRepository(t)
	leaseIdentity, err := projectgraph.NewServingIdentity("project:sales", "production", "generation-current")
	if err != nil {
		t.Fatal(err)
	}
	leaseIdentity.GenerationID = ""
	provider := &testProvider{lease: &testLease{
		identity: leaseIdentity,
		runtime:  testRuntime{model: testModel()},
	}}
