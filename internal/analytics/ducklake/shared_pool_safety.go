...
type InlineTableState struct {
	Schema         string
	Table          string
	InlinedRows    int64
	InlinedDeletes int64
}

func (s InlineTableState) HasLiveData() bool { return s.InlinedRows > 0 || s.InlinedDeletes > 0 }

type InlineFlushTarget struct {
	Schema string
	Table  string
}

type InlineFlushPlan struct {
	Targets []InlineFlushTarget
}

// InlineDataState and LegacyInlineFlushPlan are descriptive aliases kept for
// callers which use the terminology from the ADR and conformance spec.
type InlineDataState = InlineTableState
type LegacyInlineFlushPlan = InlineFlushPlan

// PlanLegacyInlineFlush produces one table-scoped flush target for each table
// with data or delete rows. The executor enables auto_compact only for the
// exact target during that flush and then persists it back to false; callers
// never rely on a global maintenance default.
func PlanLegacyInlineFlush(states []InlineTableState) (InlineFlushPlan, error) {
	targets := make([]InlineFlushTarget, 0, len(states))
	seen := map[string]struct{}{}
	for _, state := range states {
		if strings.TrimSpace(state.Schema) == "" || strings.TrimSpace(state.Table) == "" {
			return InlineFlushPlan{}, fmt.Errorf("legacy inlined table schema and table are required")
		}
		if !state.HasLiveData() {
			continue
		}
		key := strings.ToLower(strings.TrimSpace(state.Schema) + "\x00" + strings.TrimSpace(state.Table))
		if _, ok := seen[key]; ok {
			continue
		}
		seen[key] = struct{}{}
		targets = append(targets, InlineFlushTarget{Schema: strings.TrimSpace(state.Schema), Table: strings.TrimSpace(state.Table)})
	}
	sort.Slice(targets, func(i, j int) bool {
		if targets[i].Schema == targets[j].Schema {
			return targets[i].Table < targets[j].Table
		}
		return targets[i].Schema < targets[j].Schema
	})
	return InlineFlushPlan{Targets: targets}, nil
}

func PlanLegacyInlineDataFlush(states []InlineTableState) (InlineFlushPlan, error) {
	return PlanLegacyInlineFlush(states)
}

func ValidateNoLiveInlineData(states []InlineTableState) error {
	for _, state := range states {
		if state.HasLiveData() {
			return fmt.Errorf("%s.%s has %d inlined rows and %d inlined deletes: %w", state.Schema, state.Table, state.InlinedRows, state.InlinedDeletes, ErrLiveInlineData)
		}
	}
	return nil
}

func ValidateLegacyInlineDataFlushed(states []InlineTableState) error {
	return ValidateNoLiveInlineData(states)
}
