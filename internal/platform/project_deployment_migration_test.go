...
// apply/verify every migration through 096; a missing or duplicated sequence
...
// migration, then let Open apply 073..096 in one restart-safe upgrade.
...
func assertDeliveryMigrationTail(t *testing.T, ctx context.Context, store *Store) {
	...
	if latest != 96 {
		t.Fatalf("latest applied migration = %d, want 96", latest)
	}
	rows, err := store.SQLDB().QueryContext(ctx, `
		SELECT version_id
		FROM goose_db_version
		WHERE is_applied = 1 AND version_id BETWEEN 73 AND 96
		ORDER BY version_id`)
	if err != nil {
		t.Fatalf("inspect applied delivery migration sequence: %v", err)
	}
	defer rows.Close()
	for want := int64(73); want <= 96; want++ {
		if !rows.Next() {
