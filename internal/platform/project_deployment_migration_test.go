...
// apply/verify every migration through 097; a missing or duplicated sequence
...
// migration, then let Open apply 073..097 in one restart-safe upgrade.
...
func assertDeliveryMigrationTail(t *testing.T, ctx context.Context, store *Store) {
	...
	if latest != 97 {
		t.Fatalf("latest applied migration = %d, want 97", latest)
	}
	rows, err := store.SQLDB().QueryContext(ctx, `
		SELECT version_id
		FROM goose_db_version
		WHERE is_applied = 1 AND version_id BETWEEN 73 AND 97
		ORDER BY version_id`)
	if err != nil {
		t.Fatalf("inspect applied delivery migration sequence: %v", err)
	}
	defer rows.Close()
	for want := int64(73); want <= 97; want++ {
		if !rows.Next() {
	...
	for _, field := range []struct{ table, column string }{
		...
		{table: "api_tokens", column: "description"},
	} {
		var count int
