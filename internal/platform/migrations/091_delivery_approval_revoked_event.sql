...
-- Canonical approval commands use the delivery ledger as their transactional
-- command evidence. Add the terminal revocation event without weakening the
-- append-only guarantees on existing observations.
PRAGMA foreign_keys = OFF;

CREATE TABLE delivery_events_090_new (
  id TEXT PRIMARY KEY
...
INSERT INTO delivery_events_090_new (
  id, target_id, project_id, environment, actor_id, event_kind, object_kind,
  object_id, request_digest, plan_digest, result_digest, outcome, details_json,
  created_at
)
SELECT
  id, target_id, project_id, environment, actor_id, event_kind, object_kind,
...
ALTER TABLE delivery_events_090_new RENAME TO delivery_events;
...
-- Forward-only: revocation observations are immutable authorization evidence.
SELECT 1;
