...
// HTTPTransport adapts generated APIGen clients to net/http without depending
// on application composition. Capability CLI packages can therefore consume
// their generated clients while preserving the capability dependency graph.
type HTTPTransport struct {
	...
	AllowsStatus     func(operationID string, statusCode int) bool
}

func (transport HTTPTransport) DoAPIGen(
	ctx context.Context,
	...
	if request.OperationID != "" && transport.AllowsStatus != nil &&
		!transport.AllowsStatus(request.OperationID, httpResponse.StatusCode) {
		return metadata, fmt.Errorf(
