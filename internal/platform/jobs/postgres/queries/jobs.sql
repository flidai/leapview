...
-- name: RecoverExpiredRefreshJobs :execrows
WITH expired AS MATERIALIZED (
	SELECT q.id
	FROM refresh.run AS r
	JOIN jobs.job_history AS h
	  ON h.id = r.job_id
	 AND h.kind = 'refresh_pipeline'
	 AND h.resource_kind = 'refresh_run'
	 AND h.resource_id = r.run_id
	JOIN public.river_job AS q ON q.id = h.river_job_id
	WHERE r.status IN ('running', 'prepared')
	  AND r.lease_expires_at IS NOT NULL
	  AND r.lease_expires_at <= clock_timestamp()
	  AND r.lease_owner <> ''
	  AND r.fence_generation > 0
	  AND h.status = 'running'
	  AND h.attempt_count = r.fence_generation
	  AND q.state = 'running'
	  AND q.attempt = r.fence_generation
	  AND coalesce(array_length(q.attempted_by, 1), 0) > 0
	  AND q.attempted_by[array_upper(q.attempted_by, 1)] = r.lease_owner
	ORDER BY r.lease_expires_at, r.run_id
	FOR UPDATE OF r, q SKIP LOCKED
	LIMIT sqlc.arg(batch_limit)::int
)
UPDATE public.river_job AS q
SET state = 'retryable',
	scheduled_at = clock_timestamp(),
	finalized_at = NULL,
	errors = array_append(q.errors, jsonb_build_object(
		'at', clock_timestamp(),
		'attempt', q.attempt,
		'error', 'Expired refresh lease rescued by LeapView',
		'trace', ''
	)),
	metadata = q.metadata || jsonb_build_object(
		'leapview:refresh_lease_rescue_count',
		coalesce(
			CASE WHEN jsonb_typeof(q.metadata -> 'leapview:refresh_lease_rescue_count') = 'number'
				THEN (q.metadata ->> 'leapview:refresh_lease_rescue_count')::int
			END,
			0
		) + 1
	)
