...
func seedRefreshRecoveryPageSaturation(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	// Each root now owns its own project/environment publication scope. Keep
	// the 101 expired candidates that saturate the recovery page without
	// violating the one-active-root-per-scope invariant.
	run(`INSERT INTO jobs.job_history(id,kind,workload_class,principal_id,group_ids,partition_key,resource_kind,resource_id,estimated_memory_bytes,payload,request_digest,river_job_id) SELECT 'refresh-page-saturated-job-'||r.id,'refresh_pipeline','background','system','[]','refresh:page-saturation-'||r.id||':production','refresh_run','refresh-page-saturated-run-'||r.id,1,'{}','sha256:'||repeat('a',64),r.id FROM public.river_job r WHERE r.kind='refresh_pipeline' AND r.args->>'page_saturation'=$1`, marker)
	run(`INSERT INTO refresh.run(run_id,project_id,environment,generation_id,pipeline_id,semantic_model_id,target_type,target_id,trigger_type,invocation_source,plan_digest,artifact_digest,principal_id,job_id) SELECT 'refresh-page-saturated-run-'||r.id,'page-saturation-'||r.id,'production','generation-1','pipeline-1','semantic-1','refresh_pipeline','pipeline-1','manual','manual','sha256:'||repeat('a',64),'sha256:'||repeat('a',64),'system',h.id FROM public.river_job r JOIN jobs.job_history h ON h.river_job_id=r.id WHERE r.kind='refresh_pipeline' AND r.args->>'page_saturation'=$1`, marker)
	run(`UPDATE refresh.run SET status='running',attempt_count=1,fence_generation=1,lease_owner='page-saturation-dead',lease_expires_at=clock_timestamp()+interval '100 milliseconds',started_at=clock_timestamp() WHERE project_id LIKE 'page-saturation-%'; INSERT INTO refresh.attempt(run_id,attempt_number,fence_generation,owner_id,lease_expires_at) SELECT run_id,1,1,'page-saturation-dead',lease_expires_at FROM refresh.run WHERE project_id LIKE 'page-saturation-%'`)
	time.Sleep(200 * time.Millisecond)
	run(`UPDATE jobs.job_history SET status='running',attempt_count=1,started_at=clock_timestamp()-interval '10 minutes' WHERE partition_key LIKE 'refresh:page-saturation-%:production'`)
}
