...
--
-- Revision 011 owns immutable typed definitions. This migration owns only
-- control-plane rows which reference that definition registry.
...
-- A row is one assignment incarnation. Tombstones remain forever; restoring
-- a subject/definition pair creates a new immutable assignment id.
...
CREATE UNIQUE INDEX IF NOT EXISTS semantic_attribute_assignment_active_key
    ON access.semantic_attribute_assignment(definition_id, subject_kind, subject_id)
    WHERE tombstoned_at IS NULL;
CREATE INDEX IF NOT EXISTS semantic_attribute_assignment_subject_idx
...
-- is canonicalized at authentication/evaluation time.
...
CREATE UNIQUE INDEX IF NOT EXISTS semantic_attribute_claim_mapping_active_key
    ON access.semantic_attribute_claim_mapping(source_kind, provider, issuer, audience, claim, definition_id)
    WHERE tombstoned_at IS NULL;
CREATE INDEX IF NOT EXISTS semantic_attribute_claim_mapping_lookup_idx
...
END;
$$;
DROP TRIGGER IF EXISTS semantic_attribute_definition_owner_exists ON access.semantic_attribute_definition;
...
SET search_path = pg_catalog, access
AS $$
DECLARE definition_type text; definition_shape text; definition_enabled boolean; definition_version bigint; tombstone_transition boolean := false;
BEGIN
    ...
    SELECT d.value_type, d.value_shape, d.enabled, d.definition_version
      INTO definition_type, definition_shape, definition_enabled, definition_version
      FROM access.semantic_attribute_definition AS d
    ...
    -- advances. It must not be updated or resurrected, but revocation must
    -- remain possible so the active uniqueness key can be replaced.
    IF NOT tombstone_transition THEN
        IF NOT definition_enabled THEN
            RAISE EXCEPTION 'semantic attribute definition is missing or disabled';
        END IF;
        ...
        END IF;
        IF definition_version <> NEW.definition_version THEN
            RAISE EXCEPTION 'semantic attribute assignment definition version does not match the definition';
        END IF;
    END IF;
...
BEGIN
    ...
    IF NOT tombstone_transition THEN
        ...
        END IF;
        IF definition_version <> NEW.definition_version THEN
            RAISE EXCEPTION 'semantic attribute mapping definition version does not match the definition';
        END IF;
    END IF;
    IF TG_OP = 'UPDATE' THEN
...
DROP TRIGGER IF EXISTS semantic_attribute_control_state_no_delete ON access.semantic_attribute_control_state;
...
DROP TRIGGER IF EXISTS semantic_attribute_control_state_immutable ON access.semantic_attribute_control_state;
...
DROP TRIGGER IF EXISTS semantic_attribute_assignment_no_delete ON access.semantic_attribute_assignment;
CREATE TRIGGER semantic_attribute_assignment_no_delete
    BEFORE DELETE ON access.semantic_attribute_assignment
    FOR EACH ROW EXECUTE FUNCTION access.reject_access_delete();
DROP TRIGGER IF EXISTS semantic_attribute_assignment_immutable ON access.semantic_attribute_assignment;
...
DROP TRIGGER IF EXISTS semantic_attribute_claim_mapping_no_delete ON access.semantic_attribute_claim_mapping;
CREATE TRIGGER semantic_attribute_claim_mapping_no_delete
    BEFORE DELETE ON access.semantic_attribute_claim_mapping
    FOR EACH ROW EXECUTE FUNCTION access.reject_access_delete();
DROP TRIGGER IF EXISTS semantic_attribute_claim_mapping_immutable ON access.semantic_attribute_claim_mapping;
...
REVOKE ALL ON TABLE access.semantic_attribute_control_state,
    access.semantic_attribute_assignment, access.semantic_attribute_claim_mapping FROM PUBLIC;
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_runtime') THEN
        GRANT SELECT, INSERT, UPDATE ON access.semantic_attribute_control_state,
            access.semantic_attribute_assignment, access.semantic_attribute_claim_mapping TO leapview_control_runtime;
        REVOKE DELETE, TRUNCATE, REFERENCES, TRIGGER ON access.semantic_attribute_control_state,
            access.semantic_attribute_assignment, access.semantic_attribute_claim_mapping FROM leapview_control_runtime;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_readonly') THEN
        GRANT SELECT ON access.semantic_attribute_control_state,
            access.semantic_attribute_assignment, access.semantic_attribute_claim_mapping TO leapview_control_readonly;
        REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER
            ON access.semantic_attribute_control_state, access.semantic_attribute_assignment,
               access.semantic_attribute_claim_mapping FROM leapview_control_readonly;
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_backup') THEN
        GRANT SELECT ON access.semantic_attribute_control_state,
            access.semantic_attribute_assignment, access.semantic_attribute_claim_mapping TO leapview_control_backup;
    END IF;
END
$$;
