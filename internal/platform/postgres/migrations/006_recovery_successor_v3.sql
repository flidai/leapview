...
-- Additive FAI-520 persistence.  The owner copy is kept at
-- internal/recoveryset/postgres/successor_schema.sql.  This migration is
-- intentionally self-contained: Goose does not execute filesystem includes.
...
AS $$ SELECT encode(managed_data.digest(payload, 'sha256'), 'hex') $$;
-- +goose StatementEnd
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION recovery.successor_domain_sha256(domain text, payload bytea)
RETURNS text LANGUAGE sql IMMUTABLE STRICT
SET search_path = pg_catalog, recovery, managed_data
AS $$ SELECT 'sha256:' || recovery.successor_raw_sha256(convert_to(domain, 'UTF8') || payload) $$;
-- +goose StatementEnd
...
CREATE TABLE IF NOT EXISTS recovery.successor_evidence_locator_v2 (
    ...
    storage_profile_id text NOT NULL CHECK (storage_profile_id = btrim(storage_profile_id) AND storage_profile_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'),
    storage_profile_revision bigint NOT NULL CHECK (storage_profile_revision > 0),
    account_identity text NOT NULL CHECK (account_identity = btrim(account_identity) AND octet_length(account_identity) BETWEEN 1 AND 4096),
    endpoint text NOT NULL CHECK (endpoint = btrim(endpoint) AND octet_length(endpoint) BETWEEN 1 AND 2048),
    region text NOT NULL CHECK (region = btrim(region) AND octet_length(region) BETWEEN 1 AND 4096),
    bucket text NOT NULL CHECK (bucket = btrim(bucket) AND octet_length(bucket) BETWEEN 3 AND 63),
    namespace text NOT NULL CHECK (namespace = btrim(namespace) AND octet_length(namespace) BETWEEN 1 AND 4096),
    object_key text NOT NULL CHECK (object_key = btrim(object_key) AND octet_length(object_key) BETWEEN 1 AND 1024),
    version_id text NOT NULL CHECK (version_id = btrim(version_id) AND octet_length(version_id) BETWEEN 1 AND 4096),
    byte_length bigint NOT NULL CHECK (byte_length BETWEEN 2 AND 8388608),
...
BEGIN RAISE EXCEPTION 'successor recovery evidence is immutable'; END $$;
-- +goose StatementEnd
-- +goose StatementBegin
...
LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, recovery AS $$
  SELECT g.incarnation_id, g.revision, g.policy_digest FROM recovery.successor_trust_generation AS g WHERE g.singleton = true FOR UPDATE
$$;
-- +goose StatementEnd
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION recovery.guard_successor_set_identity()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, recovery AS $$
DECLARE existing_version smallint;
BEGIN
  INSERT INTO recovery.set_identity_registry(set_id, wire_version) VALUES (NEW.set_id, COALESCE(NULLIF(TG_ARGV[0], '')::smallint, 3)) ON CONFLICT (set_id) DO NOTHING;
  SELECT wire_version INTO existing_version FROM recovery.set_identity_registry WHERE set_id = NEW.set_id;
...
DECLARE existing_version smallint;
BEGIN
  INSERT INTO recovery.set_identity_registry(set_id, wire_version) VALUES (NEW.set_id, 1) ON CONFLICT (set_id) DO NOTHING;
  SELECT wire_version INTO existing_version FROM recovery.set_identity_registry WHERE set_id = NEW.set_id;
...
BEGIN
  SELECT byte_length, raw_sha256 INTO expected_length, expected_hash FROM recovery.successor_evidence_v2
   WHERE payload_family = NEW.payload_family AND payload_version = NEW.payload_version AND payload_digest = NEW.payload_digest;
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM recovery.successor_evidence_locator_v2 l JOIN recovery.recovery_set_v3 s ON s.set_id = NEW.set_id WHERE l.payload_family = s.manifest_family AND l.payload_version = s.manifest_version AND l.payload_digest = s.manifest_digest)
     OR NOT EXISTS (SELECT 1 FROM recovery.successor_evidence_locator_v2 l JOIN recovery.recovery_set_v3 s ON s.set_id = NEW.set_id WHERE l.payload_family = s.anchor_family AND l.payload_version = s.anchor_version AND l.payload_digest = s.anchor_digest)
     OR NOT EXISTS (SELECT 1 FROM recovery.successor_evidence_locator_v2 l JOIN recovery.recovery_set_v3 s ON s.set_id = NEW.set_id WHERE l.payload_family = s.profile_family AND l.payload_version = s.profile_version AND l.payload_digest = s.profile_digest)
     OR NOT EXISTS (SELECT 1 FROM recovery.successor_evidence_locator_v2 l JOIN recovery.recovery_set_v3 s ON s.set_id = NEW.set_id WHERE l.payload_family = s.receipt_family AND l.payload_version = s.receipt_version AND l.payload_digest = s.receipt_digest)
     OR NOT EXISTS (SELECT 1 FROM recovery.successor_evidence_locator_v2 l JOIN recovery.recovery_set_v3 s ON s.set_id = NEW.set_id WHERE l.payload_family = s.authority_family AND l.payload_version = s.authority_version AND l.payload_digest = s.authority_digest) THEN
...
CREATE TRIGGER successor_registry_immutable BEFORE UPDATE OR DELETE ON recovery.set_identity_registry FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();

DROP TRIGGER IF EXISTS successor_evidence_immutable ON recovery.successor_evidence_v2;
CREATE TRIGGER successor_evidence_immutable BEFORE UPDATE OR DELETE ON recovery.successor_evidence_v2 FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();
DROP TRIGGER IF EXISTS successor_locator_immutable ON recovery.successor_evidence_locator_v2;
CREATE TRIGGER successor_locator_immutable BEFORE UPDATE OR DELETE ON recovery.successor_evidence_locator_v2 FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();
DROP TRIGGER IF EXISTS successor_manifest_binding_immutable ON recovery.successor_manifest_binding;
CREATE TRIGGER successor_manifest_binding_immutable BEFORE UPDATE OR DELETE ON recovery.successor_manifest_binding FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();
DROP TRIGGER IF EXISTS successor_manifest_binding_identity ON recovery.successor_manifest_binding;
CREATE TRIGGER successor_manifest_binding_identity BEFORE INSERT ON recovery.successor_manifest_binding FOR EACH ROW EXECUTE FUNCTION recovery.guard_successor_set_identity(3);
DROP TRIGGER IF EXISTS successor_locator_guard ON recovery.successor_evidence_locator_v2;
CREATE TRIGGER successor_locator_guard BEFORE INSERT ON recovery.successor_evidence_locator_v2 FOR EACH ROW EXECUTE FUNCTION recovery.guard_successor_locator();
DROP TRIGGER IF EXISTS recovery_set_v3_insert_guard ON recovery.recovery_set_v3;
CREATE TRIGGER recovery_set_v3_insert_guard BEFORE INSERT ON recovery.recovery_set_v3 FOR EACH ROW EXECUTE FUNCTION recovery.guard_successor_set_identity(3);
DROP TRIGGER IF EXISTS recovery_set_v3_immutable ON recovery.recovery_set_v3;
CREATE TRIGGER recovery_set_v3_immutable BEFORE UPDATE OR DELETE ON recovery.recovery_set_v3 FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();
DROP TRIGGER IF EXISTS recovery_set_v3_root_immutable ON recovery.recovery_set_v3_root;
CREATE TRIGGER recovery_set_v3_root_immutable BEFORE UPDATE OR DELETE ON recovery.recovery_set_v3_root FOR EACH ROW EXECUTE FUNCTION recovery.reject_successor_mutation();
DROP TRIGGER IF EXISTS recovery_set_v3_complete ON recovery.recovery_set_v3;
CREATE CONSTRAINT TRIGGER recovery_set_v3_complete AFTER INSERT ON recovery.recovery_set_v3 DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION recovery.guard_successor_set_complete();
DROP TRIGGER IF EXISTS recovery_set_identity_registry_v1 ON recovery.recovery_set;
CREATE TRIGGER recovery_set_identity_registry_v1 BEFORE INSERT ON recovery.recovery_set FOR EACH ROW EXECUTE FUNCTION recovery.guard_v1_set_identity();

INSERT INTO recovery.set_identity_registry(set_id, wire_version)
SELECT set_id, 1 FROM recovery.recovery_set
ON CONFLICT (set_id) DO NOTHING;

CREATE INDEX IF NOT EXISTS successor_locator_profile_idx ON recovery.successor_evidence_locator_v2(storage_profile_id, account_identity, endpoint, bucket);
CREATE INDEX IF NOT EXISTS recovery_set_v3_manifest_idx ON recovery.recovery_set_v3(manifest_digest, created_at DESC);

REVOKE ALL ON TABLE recovery.set_identity_registry, recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding, recovery.successor_trust_generation FROM PUBLIC;
REVOKE ALL ON FUNCTION recovery.successor_raw_sha256(bytea), recovery.successor_domain_sha256(text, bytea), recovery.reject_successor_mutation(), recovery.lock_successor_generation(), recovery.guard_successor_set_identity(), recovery.guard_v1_set_identity(), recovery.guard_successor_locator(), recovery.guard_successor_set_complete() FROM PUBLIC;
-- +goose StatementBegin
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_maintenance') THEN
    ...
    GRANT SELECT, INSERT ON recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding TO leapview_control_maintenance;
    ...
    REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding FROM leapview_control_maintenance;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_runtime') THEN
    REVOKE ALL ON recovery.set_identity_registry, recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding, recovery.successor_trust_generation FROM leapview_control_runtime;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_readonly') THEN
    GRANT SELECT ON recovery.set_identity_registry, recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding, recovery.successor_trust_generation TO leapview_control_readonly;
    REVOKE INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON recovery.set_identity_registry, recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding, recovery.successor_trust_generation FROM leapview_control_readonly;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'leapview_control_backup') THEN
    GRANT SELECT ON recovery.set_identity_registry, recovery.successor_evidence_v2, recovery.successor_evidence_locator_v2, recovery.recovery_set_v3, recovery.recovery_set_v3_root, recovery.successor_manifest_binding, recovery.successor_trust_generation TO leapview_control_backup;
...
-- +goose StatementBegin
DO $$ BEGIN
  RAISE EXCEPTION 'recovery successor v3 migration is immutable; destructive down is forbidden';
END $$;
-- +goose StatementEnd
RESET ROLE;
