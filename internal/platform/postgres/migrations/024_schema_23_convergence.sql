...
-- Converge both known revision-23 histories. The main lineage already has
-- these recovery-qualification objects; the live audit lineage does not.
-- Every data-bearing object is created additively, while functions, triggers,
-- and grants are reconciled to the current contract without removing rows.
...
-- +goose Down
SET LOCAL ROLE leapview_control_owner;
-- Revision 024 reconciles two distinct historical layouts recorded by Goose
-- as revision 023. Reversing it cannot identify which lineage preceded the
-- convergence and could destroy durable recovery evidence.
-- +goose StatementBegin
DO $$ BEGIN
    RAISE EXCEPTION 'schema 24 convergence is forward-only; destructive down is forbidden';
END $$;
-- +goose StatementEnd
RESET ROLE;
