...
-- The v1 token migration deliberately shipped a bounded action list, but its
-- JSON extraction treated omitted/null fields as SQL NULL and therefore could
-- accept malformed objects.  Keep the historical migration unchanged and
-- replace only the live validator with an exact, fail-closed v1 contract.
...
DO $$
BEGIN
    RAISE EXCEPTION 'typed permission validation hardening is immutable';
END $$;
-- +goose StatementEnd
RESET ROLE;
