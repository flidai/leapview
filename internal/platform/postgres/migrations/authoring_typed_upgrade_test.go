...
func TestTypedAuthoringPermissionsMigrationRetiresLegacyScopes(t *testing.T) {
	...
	for revision := 2; revision <= 40; revision++ {
	...
	if _, err := provider.UpTo(t.Context(), 40); err != nil {
		t.Fatalf("apply authoring fixture revision 40: %v", err)
	}
	...
	upgrade["041_typed_authoring_permissions.sql"] = &fstest.MapFile{Data: contents}
