...
func TestEmbeddedGooseBaselineIsImmutableAndForwardMigrationsAreOrdered(t *testing.T) {
	...
	if got, want := strings.Join(sqlFiles, ","), "001_control_plane.sql,002_project_free_source_bundle.sql,003_saved_explorations.sql"; got != want {
...
func TestSavedExplorationMigrationMirrorsNativeSchemaAndRoleFence(t *testing.T) {
	contents, err := fs.ReadFile(MigrationFS(), "003_saved_explorations.sql")
	if err != nil {
		t.Fatal(err)
	}
	text := string(contents)
