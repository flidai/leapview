...
func TestOpenLineageHasOneProjectionAndNoTransportPath(t *testing.T) {
	...
	for _, file := range files {
		if strings.Contains(file.pkgDir, "lineage") && file.pkgDir != openLineageProjectionPackage {
			t.Errorf("%s creates a second lineage package outside %s", file.pkgDir, openLineageProjectionPackage)
		}
		if file.pkgDir != openLineageProjectionPackage {
			for _, imported := range file.imports {
