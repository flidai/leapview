...
type SemanticModelVisibility func(context.Context, Lease, projectgraph.ResourceID) (bool, error)
...
func (s *Service) semanticModelAllowed(ctx context.Context, lease Lease, resource projectgraph.Resource) (bool, error) {
	...
	return s.semanticModelVisibility(ctx, lease, resource.ID)
}
...
func (s *Service) Search(ctx context.Context, request SearchRequest) (Page, error) {
	...
	for _, resource := range graph.Resources() {
		...
		semanticAllowed, err := s.semanticModelAllowed(ctx, lease, resource)
		if err != nil {
			return Page{}, err
		}
		if !semanticAllowed {
