...
func projectRouteUpdatesURL(routeKind uisignals.RouteKind, catalog catalog.Catalog, page any, extras projectDocumentExtras) string {
	...
	case uisignals.ResourcePageSignal:
		...
		return updatesURL(routeKind, "environment", uisignals.ValueOrZero(typed.Environment), "type", firstNonEmpty(uisignals.ValueOrZero(assetList.ActiveType), uisignals.ValueOrZero(typed.ListFilter)), "q", firstNonEmpty(uisignals.ValueOrZero(assetList.Query), uisignals.ValueOrZero(typed.ListQuery)))
	...
	case uisignals.ResourceAssetPageSignal:
		if routeKind == uisignals.RouteKindConnectionAsset {
			return updatesURL(routeKind, "environment", uisignals.ValueOrZero(typed.Environment), "asset", typed.AssetID, "section", typed.ActiveSection)
		}
		pairs := []string{"environment", uisignals.ValueOrZero(typed.Environment), "asset", typed.AssetID, "section", typed.ActiveSection}
