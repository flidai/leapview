...
func (h *BrowserHandler) SavedExplorationReopen(w stdhttp.ResponseWriter, r *stdhttp.Request) {
	...
	page, explorer, ok := h.dataExplorerSignalsForCommandWithOptions(w, r, command, executeQuery, false)
	if !ok {
		return
	}
...
func (h *BrowserHandler) savedMutationReplayAuthorizationRequest(projectID projectgraph.ResourceID, actor string, target saved.ExplorationID, command projectsignals.SavedExplorationCommandSignal, revision saved.RevisionToken, key string) (saved.MutationReplayAuthorizationRequest, error) {
	request := saved.MutationReplayAuthorizationRequest{ProjectID: projectID, ActorID: actor, IdempotencyKey: key}
	switch command.Action {
	case "create":
		spec, payload, err := savedCommandSpec(command)
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
		id, err := saved.StableExplorationID("exploration-", projectID.String(), actor, key, "create")
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
		...
		fingerprint, err := saved.FingerprintCreate(create)
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
	...
	case "update":
		...
		fingerprint, err := saved.FingerprintUpdate(update)
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
	...
	case "duplicate":
		...
		id, err := saved.StableExplorationID("exploration-", projectID.String(), actor, key, "duplicate:"+target.String())
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
		duplicate.Slug = savedBrowserCommandSlug(command.Slug, duplicate.Title, id)
		duplicate.ID = saved.ExplorationID(id)
		fingerprint, err := saved.FingerprintDuplicate(duplicate)
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
	...
	case "archive":
		...
		fingerprint, err := saved.FingerprintArchive(archive)
		if err != nil {
			return saved.MutationReplayAuthorizationRequest{}, err
		}
		...
		request.Fingerprint = fingerprint
	default:
		return saved.MutationReplayAuthorizationRequest{}, saved.ErrInvalid
	}
	return request, nil
}
...
func (h *BrowserHandler) savedCreate(ctx context.Context, projectID projectgraph.ResourceID, actor string, command projectsignals.SavedExplorationCommandSignal, key, requestID, correlationID string) (saved.MutationResult, error) {
	...
	fingerprint, err := saved.FingerprintCreate(request)
	if err != nil {
		return saved.MutationResult{}, err
	}
...
func (h *BrowserHandler) savedUpdate(ctx context.Context, projectID projectgraph.ResourceID, actor string, target saved.ExplorationID, command projectsignals.SavedExplorationCommandSignal, revision saved.RevisionToken, key, requestID, correlationID string) (saved.MutationResult, error) {
	...
	fingerprint, err := saved.FingerprintUpdate(request)
	if err != nil {
		return saved.MutationResult{}, err
	}
...
func (h *BrowserHandler) savedDuplicate(ctx context.Context, projectID projectgraph.ResourceID, actor string, source saved.ExplorationID, command projectsignals.SavedExplorationCommandSignal, revision saved.RevisionToken, key, requestID, correlationID string) (saved.MutationResult, error) {
	...
	id, err := saved.StableExplorationID("exploration-", projectID.String(), actor, key, "duplicate:"+source.String())
	if err != nil {
		return saved.MutationResult{}, err
	}
	request.Slug = savedBrowserCommandSlug(command.Slug, title, id)
	request.ID = saved.ExplorationID(id)
	fingerprint, err := saved.FingerprintDuplicate(request)
	if err != nil {
		return saved.MutationResult{}, err
	}
...
func (h *BrowserHandler) savedArchive(ctx context.Context, projectID projectgraph.ResourceID, actor string, target saved.ExplorationID, revision saved.RevisionToken, key, requestID, correlationID string) (saved.MutationResult, error) {
	...
	fingerprint, err := saved.FingerprintArchive(request)
	if err != nil {
		return saved.MutationResult{}, err
	}
