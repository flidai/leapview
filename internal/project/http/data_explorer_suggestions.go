...
func dataExplorerFilterSuggestions(ctx context.Context, executor DataQueryExecutor, lowerer exploration.QueryLowerer, projectID projectgraph.ResourceID, command projectsignals.DataExploreCommand, fields []projectsignals.DataExploreFieldSignal, compiled *semanticquery.CompiledModel) (signal projectsignals.DataExploreFilterSuggestionsSignal) {
	...
	filters, err := lowerSuggestionFilters(command.Spec, fieldByID, fieldID, search, lowerer)
	if err != nil {
		signal.Error = projectsignals.Pointer(err.Error())
		return signal
	}
...
func lowerSuggestionFilters(spec exploration.ExplorationSpec, fields map[string]projectsignals.DataExploreFieldSignal, suggestionField, search string, lowerer exploration.QueryLowerer) ([]dataquery.Filter, error) {
	if lowerer == nil {
		return nil, errors.New("governed exploration query lowering is unavailable")
	}
	spec = normalizeExplorationSpec(spec)
	...
	filters, err := lowerer.Filters(loweredSpec)
	if err != nil {
		return nil, err
	}
	if search == "" {
...
func suggestionSearchValue(search, logicalType string) (any, error) {
	...
	return lowerSuggestionFilterValue(value)
}

func lowerSuggestionFilterValue(value exploration.ExplorationFilterValue) (any, error) {
	...
	case *exploration.TimestampExplorationFilterValue:
		return typed.Value, nil
	default:
		return nil, fmt.Errorf("unsupported suggestion value %T", typed)
	}
}
