...
func (h Handler) globalDataExplorerStateWithCurrent(r *nethttp.Request, command uisignals.DataExplorerCommand, current *uisignals.DataExplorerSignal) (uisignals.DataExplorerPageSignal, uisignals.DataExplorerSignal, error) {
	...
	// The process serves one active project. Resolve it from the injected
	// server catalog rather than allowing browser commands or query parameters
	// to select a project.
	projectCatalog := h.ReadModel.rootCatalog()
	projectID := strings.TrimSpace(projectCatalog.Project.ID)
	if projectID == "" {
		return uisignals.DataExplorerPageSignal{}, uisignals.DataExplorerSignal{}, fmt.Errorf("active project catalog is unavailable")
	}
	projectTitle := firstNonEmpty(projectCatalog.Project.Title, projectID)
	environment := string(h.environment(r))
	objects := []uisignals.DataExplorerObjectSignal{}
	warnings := []string{}
	metrics, ok := h.metricsForProject(projectID)
	if !ok || metrics == nil {
		warnings = append(warnings, fmt.Sprintf("Project %q metrics are not configured.", projectID))
	} else {
		assets, edges, err := h.projectAssetsAndEdgesForData(r.Context(), projectID, environment)
		if err != nil {
			fallback := dataExplorerObjectsFromMetrics(projectID, projectTitle, metrics)
			if len(fallback) == 0 {
				warnings = append(warnings, fmt.Sprintf("Project %q assets are unavailable: %v", projectID, err))
			}
			objects = append(objects, fallback...)
		} else {
			projectObjects, objectWarnings := dataExplorerObjects(projectID, projectTitle, metrics, assets, edges)
			objects = append(objects, projectObjects...)
			warnings = append(warnings, objectWarnings...)
		}
	}
	selected, selectionWarnings := selectGlobalDataExplorerObject(objects, uisignals.ValueOrZero(command.ObjectKey))
	...
	exploreCommand := normalizeDataExploreCommand(*command.Explore)
	command.Explore = &exploreCommand
	explore := h.dataExploreState(r.Context(), projectID, exploreCommand)
	...
	if selected != nil {
		...
		} else if metrics, ok := h.metricsForProject(projectID); ok && metrics != nil {
			explorer.Preview = h.dataPreview(r.Context(), projectID, metrics, copy, command, current)
		} else {
			explorer.Preview.Error = uisignals.Pointer(fmt.Sprintf("project %q metrics are not configured", projectID))
		}
	}
	page := uisignals.DataExplorerPageSignal{
		...
		Tabs: []uisignals.ResourceTabSignal{
			{ID: "all", Label: "All", Href: "/explore", Active: true},
		},
	}
	return page, explorer, nil
}

func (h Handler) dataExploreState(ctx context.Context, projectID string, command uisignals.DataExploreCommand) uisignals.DataExploreSignal {
	...
	metrics, ok := h.metricsForProject(projectID)
	if !ok || metrics == nil {
		state.Result.Error = uisignals.Pointer(fmt.Sprintf("project %q metrics are not configured", projectID))
		return state
	}
	...
	if len(state.Models) == 0 {
		state.Result.Error = uisignals.Pointer("No semantic models are available in this project.")
		return state
	}
	...
	request := DataExploreRequest{
		ProjectID: projectID, ModelID: modelID, DatasetID: datasetID,
...
func dataExploreObjectForDataset(objects []uisignals.DataExplorerObjectSignal, command uisignals.DataExploreCommand) *uisignals.DataExplorerObjectSignal {
	...
	for index := range objects {
		...
		if object.Layer == "model_table" && uisignals.ValueOrZero(object.ModelID) == modelID && uisignals.ValueOrZero(object.Table) == datasetID {
			return object
		}
	}
	return nil
...
func dataExplorerObjects(projectID, projectTitle string, metrics Metrics, assets []project.DevelopAssetView, edges []project.DevelopEdgeView) ([]uisignals.DataExplorerObjectSignal, []string) {
	out := []uisignals.DataExplorerObjectSignal{}
	warnings := []string{}
	for _, asset := range assets {
		modelID, name := keyParts(asset.Key)
		switch asset.Type {
		case string(project.AssetTypeSource):
			sourceModelID, sourceName, source, ok := dataExplorerSourceForAsset(metrics, asset.Key)
			if !ok {
				warnings = append(warnings, fmt.Sprintf("Source %q is not available in the project runtime catalog.", asset.Title))
				continue
			}
			...
			out = append(out, uisignals.DataExplorerObjectSignal{
				...
				DetailHref:    uisignals.Optional(assetnav.CanonicalAssetSectionHref(asset, "details")),
...
func dataExplorerObjectsFromMetrics(projectID, projectTitle string, metrics Metrics) []uisignals.DataExplorerObjectSignal {
	out := []uisignals.DataExplorerObjectSignal{}
	for _, modelSummary := range metrics.Catalog().Models {
		model, ok := metrics.DataExplorerModel(modelSummary.ID)
		if !ok {
...
func selectGlobalDataExplorerObject(objects []uisignals.DataExplorerObjectSignal, key string) (*uisignals.DataExplorerObjectSignal, []string) {
...
func (h Handler) dataPreview(ctx context.Context, projectID string, metrics Metrics, object uisignals.DataExplorerObjectSignal, command uisignals.DataExplorerCommand, current *uisignals.DataExplorerSignal) uisignals.DataPreviewSignal {
	preview := uisignals.DataPreviewSignal{
	...
	if totals, ok := reusableDataPreviewTotals(current, object, command); ok {
		...
		total, err := h.countDataPreview(ctx, projectID, metrics, object)
		if err != nil {
	...
	for index, blockID := range blockIDs {
		...
		rows, sqlText, err := h.previewRows(ctx, projectID, metrics, object, command, start, int(command.Count))
		if sqlText != "" {
...
func reusableDataPreviewTotals(current *uisignals.DataExplorerSignal, object uisignals.DataExplorerObjectSignal, command uisignals.DataExplorerCommand) (uisignals.DataPreviewSignal, bool) {
	...
	if current.SelectedObject.Key != object.Key {
		return uisignals.DataPreviewSignal{}, false
	}
...
func (h Handler) countDataPreview(ctx context.Context, projectID string, metrics Metrics, object uisignals.DataExplorerObjectSignal) (string, error) {
	switch object.Layer {
	case "model_table":
		result, err := metrics.ExecuteDataPreview(ctx, dataPreviewQuery(projectID, object, uisignals.DataExplorerCommand{}, 0, 1, true))
		if err != nil {
			return "Unknown", err
		}
...
func (h Handler) previewRows(ctx context.Context, projectID string, metrics Metrics, object uisignals.DataExplorerObjectSignal, command uisignals.DataExplorerCommand, start, count int) ([]map[string]any, string, error) {
	result, err := metrics.ExecuteDataPreview(ctx, dataPreviewQuery(projectID, object, command, start, count, false))
	if err != nil {
		return nil, "", err
	}
...
func dataPreviewQuery(projectID string, object uisignals.DataExplorerObjectSignal, command uisignals.DataExplorerCommand, start, count int, includeTotal bool) DataPreviewRequest {
