...
func (h Handler) DataExplorer(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if err := ui.DataExplorerPageWithAgent(h.catalogForProjectsPage(r, nil), page, explorer, h.dataExplorerAgentBootstrap(r, explorer), h.AgentCommands, h.csrfToken(r), h.chromeOptions(r)...).Render(w); err != nil {
		nethttp.Error(w, err.Error(), nethttp.StatusInternalServerError)
	}
}

func (h Handler) ProjectDataExplorerRedirect(w nethttp.ResponseWriter, r *nethttp.Request) {
	values := url.Values{}
...
func (h Handler) DataExplorerUpdates(w nethttp.ResponseWriter, r *nethttp.Request) {
	...
	if err := updates.Patch(ui.DataExplorerBootstrapSignalsWithAgent(h.catalogForProjectsPage(r, nil), page, explorer, h.dataExplorerAgentBootstrap(r, explorer), h.chromeOptions(r)...)); err != nil {
		return
	}
	if broker != nil {
...
func dataExplorerCommandFromQuery(_ string, object string) uisignals.DataExplorerCommand {
...
func dataExplorerCommandFromURL(values url.Values) uisignals.DataExplorerCommand {
	command := dataExplorerCommandFromQuery(values.Get("project"), values.Get("object"))
...
func DataExplorerCommandFromQuery(projectID, object string) uisignals.DataExplorerCommand {
	return dataExplorerCommandFromQuery(projectID, object)
}
...
func normalizeDataExplorerCommand(command uisignals.DataExplorerCommand) uisignals.DataExplorerCommand {
	...
	explore := normalizeDataExploreCommand(*command.Explore)
	command.Explore = &explore
	return command
}

func normalizeDataExploreCommand(command uisignals.DataExploreCommand) uisignals.DataExploreCommand {
