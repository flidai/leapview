...
func TestBundlePlanCompilerDeclaresDeterminismFromVolatileExpressions(t *testing.T) {
	deterministic := sourceAssembly{Connections: map[string]semanticmodel.Connection{"files": {Kind: "managed"}}, Sources: map[string]semanticmodel.Source{"orders": {Connection: "files"}}, Models: map[string]semanticmodel.Table{
		"orders": {Execution: semanticmodel.ExecutionDefinition{Source: "orders"}, SourceDependencies: []string{"orders"}},
	}}
	if plan := planForSourceAssembly(deterministic); !plan.Deterministic {
	...
	for _, expression := range []string{"SELECT id FROM source.orders", "SELECT safe_future_function(id) FROM source.orders", "SELECT now() AS observed_at", "SELECT random() AS sample", "SELECT uuid() AS key"} {
		...
		if plan := planForSourceAssembly(volatile); plan.Deterministic {
...
func TestPlanSourceRootAgainstBundleDetectsSQLChangeWithStableGraphIdentity(t *testing.T) {
	...
	retained, err := LoadSourceRoot(projectPath)
	if err != nil {
		t.Fatal(err)
	}
	active, err := projectartifact.NewSourceBundle(retained.Graph, retained.Manifest)
	if err != nil {
		t.Fatal(err)
	}
	modelPath := filepath.Join(projectPath, "models", "orders.yaml")
	...
	plan, err := PlanSourceRootAgainstBundle(projectPath, active)
	if err != nil {
		t.Fatal(err)
	}
