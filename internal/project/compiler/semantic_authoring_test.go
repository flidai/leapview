  ...
  datasets:
    - name: orders
      ...
      dimensions:
        ...
        - {name: order_status, field: status}
      metrics:
        - {name: amount, type: simple, agg: sum}
        - {name: orders, type: simple, agg: count_distinct, field: id, timeDimension: ordered_at}
  metrics:
    - {name: average_amount, type: ratio, numerator: amount, denominator: orders}
`))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDatasetLocalAuthoringRejectsDuplicateMembers(t *testing.T) {
	for _, tc := range []struct{ name, body, want string }{
		{"metric across scopes", "datasets: [{name: orders, model: orders_model, metrics: [{name: revenue, type: simple, agg: sum}]}]\nmetrics: [{name: revenue, type: derived, expression: revenue * 2}]", "datasets.orders.metrics.revenue conflicts with spec.metrics.revenue"},
		{"metric across datasets", "datasets: [{name: orders, model: orders_model, metrics: [{name: revenue, type: simple, agg: sum}]}, {name: refunds, model: refunds_model, metrics: [{name: revenue, type: simple, agg: sum}]}]", "datasets.refunds.metrics.revenue conflicts with datasets.orders.metrics.revenue"},
		{"dimension across scopes", "datasets: [{name: orders, model: orders_model, dimensions: [{name: status}]}]\ndimensions: [{name: status, bindings: [{dataset: orders, field: orders.status}]}]", "datasets.orders.dimensions.status conflicts with spec.dimensions.status"},
	} {
...
func TestDatasetLocalDatatypeAssertionsAndDiscovery(t *testing.T) {
	for _, tc := range []struct {
		...
		{"inferred", "{name: amount}", "Decimal", "", false},
		{"assertion mismatch", "{name: amount, datatype: String}", "Decimal", "incompatible with binding", false},
		{"unresolved at activation", "{name: amount}", "", "requires a logical datatype", true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			yaml := "apiVersion: leapview.dev/v1\nkind: SemanticModel\nmetadata: {id: semantic-model:sales, name: sales}\nspec:\n  datasets:\n    - name: orders\n      model: orders_model\n      dimensions:\n        - " + tc.dimension + "\n      metrics:\n        - {name: amount_sum, type: simple, agg: sum, field: amount}\n"
