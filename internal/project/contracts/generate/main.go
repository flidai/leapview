...
func derivePathFormatOptions(doc document) ([]pathFormatOption, error) {
	...
	for _, variant := range pathSchema.OneOf {
		...
		property, hasProperty := defaults.Properties[format]
		profileRef := ""
		profileSchema := variantSchema
		if hasProperty {
			if property.Schema.Ref == "" {
				return nil, fmt.Errorf("ReaderDefaults option %q has no typed model reference", format)
			}
			profileRef = property.Schema.Ref
			var found bool
			profileSchema, found = doc.Schemas[profileRef]
			if !found {
				return nil, fmt.Errorf("format %q option model %q is missing from APIGen IR", format, profileRef)
			}
		}
