...
func main() {
	...
	if err := patchFreshnessTS(tsOut); err != nil {
		fmt.Fprintln(os.Stderr, "generate TypeScript freshness contract:", err)
		os.Exit(1)
	}
...
func patchFreshnessSchema(path string) error {
	...
	for _, name := range []string{"FieldFreshness", "RevisionFreshness"} {
		definition, ok := defs[name].(map[string]any)
		if !ok {
			return fmt.Errorf("generated schema has no %s definition", name)
		}
		definition["anyOf"] = []any{
			map[string]any{"required": []any{"warningAfter"}},
			map[string]any{"required": []any{"errorAfter"}},
		}
	}
	encoded, err := json.MarshalIndent(document, "", "  ")
	if err != nil {
...
func patchFreshnessTS(path string) error {
	...
	text := string(content)
	if strings.Contains(text, "export type FieldFreshness = FieldFreshnessWarning | FieldFreshnessError") {
		return nil
	}
	var errReplace error
	text, errReplace = replaceTSBlock(text, "export interface FieldFreshness {", "export interface FieldFreshnessBase {\n  basis: 'field'\n  field: string\n}\n\nexport interface FieldFreshnessWarning extends FieldFreshnessBase {\n  warningAfter: FreshnessDuration\n  errorAfter?: FreshnessDuration\n}\n\nexport interface FieldFreshnessError extends FieldFreshnessBase {\n  warningAfter?: FreshnessDuration\n  errorAfter: FreshnessDuration\n}\n\nexport type FieldFreshness = FieldFreshnessWarning | FieldFreshnessError\n")
	if errReplace != nil {
		return errReplace
	}
	text, errReplace = replaceTSBlock(text, "export interface RevisionFreshness {", "export interface RevisionFreshnessBase {\n  basis: 'revision'\n  revision: string\n}\n\nexport interface RevisionFreshnessWarning extends RevisionFreshnessBase {\n  warningAfter: FreshnessDuration\n  errorAfter?: FreshnessDuration\n}\n\nexport interface RevisionFreshnessError extends RevisionFreshnessBase {\n  warningAfter?: FreshnessDuration\n  errorAfter: FreshnessDuration\n}\n\nexport type RevisionFreshness = RevisionFreshnessWarning | RevisionFreshnessError\n")
	if errReplace != nil {
		return errReplace
	}
	text, errReplace = replaceTSBlock(text, "export interface SourceFreshnessFieldVariant extends FieldFreshness {", "export type SourceFreshnessFieldVariant = FieldFreshness\n")
	if errReplace != nil {
		return errReplace
	}
	text, errReplace = replaceTSBlock(text, "export interface SourceFreshnessRevisionVariant extends RevisionFreshness {", "export type SourceFreshnessRevisionVariant = RevisionFreshness\n")
	if errReplace != nil {
		return errReplace
	}
	return os.WriteFile(path, []byte(text), 0o644)
...
func replaceTSBlock(text, marker, replacement string) (string, error) {
	start := strings.Index(text, marker)
	if start < 0 {
		return text, fmt.Errorf("generated TypeScript has no %s block", strings.TrimSuffix(marker, " {"))
	}
	end := strings.Index(text[start:], "\n}\n")
	if end < 0 {
		return text, fmt.Errorf("generated TypeScript %s block is malformed", strings.TrimSuffix(marker, " {"))
	}
	end = start + end + len("\n}\n")
