...
// DefaultPathSourceOptions returns the generated, versioned defaults
// for a supported path format. Runtime code must use this registry
// instead of maintaining a parallel format list.
func DefaultPathSourceOptions(format string) (any, bool) {
	switch format {
	case "csv":
		return map[string]any{"delimiter": ",", "escape": "\"", "header": false, "quote": "\""}, true
	case "json":
		return map[string]any{"format": "auto"}, true
	case "parquet":
		return map[string]any{"hivePartitioning": false, "unionByName": false}, true
	case "excel":
		return map[string]any{"header": true}, true
	case "text":
		return map[string]any{"delimiter": "\t", "header": false, "quote": "\""}, true
	case "blob":
		return map[string]any{"compression": "auto"}, true
	case "vortex":
