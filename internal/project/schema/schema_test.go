...
func TestSemanticModelSchemaAcceptanceMatrix(t *testing.T) {
	t.Parallel()
	// Generated TypeSpec JSON Schema is authoritative after migration. Keep
	// datasets non-empty because that is the exported-schema/compiler-effective
	// rule, even though the legacy CUE definition did not encode minProperties.
