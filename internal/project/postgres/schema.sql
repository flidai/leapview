...
-- Immutable contract publication authority. Publication identity is scoped by
-- the instance and authored graph identity; it deliberately does not depend
-- on a separate instance-local lifecycle ledger. Canonical bytes are the
-- source of truth: every denormalized identity field and the digest must
-- agree with the exact stored bytes.
