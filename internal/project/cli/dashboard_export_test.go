...
func TestDashboardExportRequiresLocalProjectAndPreservesExistingOutputs(t *testing.T) {
	...
	command.SetArgs([]string{"sales", "--layout", "fragmented", "--source-root", projectPath})
...
func TestDashboardExportFragmentedCLIWritesReviewableSources(t *testing.T) {
	...
	write("semantic-models/sales.yaml", "apiVersion: leapview.dev/v1\nkind: SemanticModel\nmetadata: { id: semantic:sales, name: sales }\nspec: {datasets: [{name: orders, model: orders_model, metrics: [{name: order_count, type: simple, empty: zero, agg: count, field: id}]}]}\n")
	...
	command.SetArgs([]string{"dashboard:sales", "--source-root", root, "--layout", "fragmented", "--out", output})
