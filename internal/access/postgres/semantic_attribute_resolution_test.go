...
func TestResolveSemanticAttributesPostgreSQL18ReturnsCoherentRegistryControlAndDirectGroupValues(t *testing.T) {
	...
	if resolved.ControlState.Revision != 2 || resolved.ControlState.Digest == "" {
		t.Fatalf("control state = %#v, want two assignments at revision 2", resolved.ControlState)
	}
...
func TestResolveSemanticAttributesPostgreSQL18RejectsInactivePrincipal(t *testing.T) {
	db := newAuditDatabase(t)
	repo, err := NewAccess(db.runtime, FingerprintConfig{Key: []byte("0123456789abcdef0123456789abcdef")})
	if err != nil {
		t.Fatal(err)
	}
	if err := repo.DeletePrincipal(t.Context(), auditActorID); err != nil {
		t.Fatal(err)
	}
	...
	if !errors.Is(err, access.ErrSemanticAttributeSourceConflict) {
		t.Fatalf("inactive principal error = %v, want source conflict", err)
	}
}
