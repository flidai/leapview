...
INSERT INTO access.device_authorization
    (id, client_id, device_code_hash, user_code_hash, target_id, project_id,
     permission_profile, permissions, status, expires_at, created_at, poll_interval_seconds)
SELECT sqlc.arg(id), sqlc.arg(client_id), sqlc.arg(device_code_hash), sqlc.arg(user_code_hash),
       sqlc.arg(target_id), sqlc.arg(project_id), sqlc.arg(permission_profile), sqlc.arg(permissions)::jsonb, sqlc.arg(status),
...
SELECT id, client_id, device_code_hash, user_code_hash, target_id, project_id,
       permission_profile, permissions, status, principal_id, expires_at, poll_interval_seconds,
...
INSERT INTO access.authoring_session
    (id, kind, client_id, principal_id, target_id, project_id, permission_profile, permissions, created_at, expires_at)
VALUES (sqlc.arg(id), sqlc.arg(kind), sqlc.arg(client_id), sqlc.arg(principal_id)::uuid,
        sqlc.arg(target_id), sqlc.arg(project_id), sqlc.arg(permission_profile), sqlc.arg(permissions)::jsonb,
...
SELECT c.id, c.session_id, c.access_expires_at,
       ...
       s.permission_profile, s.permissions, s.expires_at, s.created_at, s.revoked_at
...
SELECT c.id, c.session_id, s.principal_id, s.kind, s.client_id, s.target_id,
       s.project_id, s.permission_profile, s.permissions, c.access_expires_at, s.expires_at
FROM access.authoring_credential c
JOIN access.authoring_session s ON s.id = c.session_id
...
SELECT id, kind, client_id, target_id, project_id, permission_profile, permissions,
