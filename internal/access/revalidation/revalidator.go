...
// callerAuthorityRevalidator is the live check for caller
...
func NewCallerAuthorityRevalidator(tokens access.APITokenAuthorityEvidenceReader, sessions access.SessionAuthorityEvidenceReader, current func(context.Context, string, access.PermissionPair, string) (bool, error), requirement access.TypedOperationRequirement, requirementErr error) jobs.AuthorityRevalidator {
...
func (r callerAuthorityRevalidator) Revalidate(ctx context.Context, authority jobs.AuthorityEnvelope) error {
	...
	if r.requirementErr != nil {
		return fmt.Errorf("%w: typed operation requirement: %v", jobs.ErrAuthorityInvalid, r.requirementErr)
	}
