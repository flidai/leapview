...
func (h Handler) upsertSemanticAttributeAssignment(w stdhttp.ResponseWriter, r *stdhttp.Request, kind access.SubjectKind, operation accessgen.GenCommandOperationID) {
	...
	err = executeSemanticAttributeMutation(r, operation, semanticAttributeAssignmentETag(current), func() error {
		var callErr error
...
func (h Handler) UpsertSemanticAttributeClaimMapping(w stdhttp.ResponseWriter, r *stdhttp.Request) {
	...
	if err := trustedclaims.ValidateSourceIdentity(input.Provider, input.Issuer, input.Audience); err != nil {
		writeSemanticAttributeError(w, invalidSemanticAttributeRequest(err))
		return
	}
	if err := trustedclaims.ValidateClaimName(input.Claim); err != nil {
		writeSemanticAttributeError(w, invalidSemanticAttributeRequest(err))
		return
	}
	repo, err := h.repository()
	...
	err = executeSemanticAttributeMutation(r, accessgen.GenCommandOperationUpsertSemanticAttributeClaimMapping(), semanticAttributeMappingETag(current), func() error {
		var callErr error
...
func (h Handler) RemoveSemanticAttributeClaimMapping(w stdhttp.ResponseWriter, r *stdhttp.Request) {
	...
	err = executeSemanticAttributeMutation(r, accessgen.GenCommandOperationRemoveSemanticAttributeClaimMapping(), semanticAttributeMappingETag(current), func() error {
...
func executeSemanticAttributeMutation(r *stdhttp.Request, operation accessgen.GenCommandOperationID, currentRevision string, mutation func() error) error {
	...
	contract, ok := accessgen.GetAPIGenCommandRuntimeContract(operation.APIGenOperationID())
	if !ok {
		return fmt.Errorf("generated command contract for %q is unavailable", operation.APIGenOperationID())
	}
...
func writeSemanticAttributeError(w stdhttp.ResponseWriter, err error) {
	...
	case errors.Is(err, access.ErrSemanticAttributeControlCorrupt):
		status = stdhttp.StatusServiceUnavailable
	case errors.Is(err, errSemanticAttributeInvalid), errors.Is(err, access.ErrSemanticAttributeSearchInvalid), errors.Is(err, semanticvalue.ErrInvalidValue), errors.Is(err, semanticvalue.ErrInvalidType), errors.Is(err, access.ErrInvalidSubjectRef), errors.Is(err, access.ErrSemanticAttributeDisabled), errors.Is(err, trustedclaims.ErrTrustIdentityMissing), errors.Is(err, trustedclaims.ErrClaimNameInvalid), errors.Is(err, trustedclaims.ErrUnsupportedSource):
