...
func TestSemanticAttributeHTTPDefinitionListMapsInvalidSearchToBadRequest(t *testing.T) {
	for name, query := range map[string]string{
		"query":  "q=" + strings.Repeat("x", 256),
		"cursor": "pageToken=" + encodeKeyCursor("region\x00not-a-uuid"),
	} {
		t.Run(name, func(t *testing.T) {
			repo := semanticAttributeHTTPRepository()
			repo.searchErr = fmt.Errorf("search failed: %w", access.ErrSemanticAttributeSearchInvalid)
			handler := semanticAttributeHTTPHandler(repo, true)
			request := httptest.NewRequest(http.MethodGet, "/api/v1/semantic-attributes?"+query, nil)
			response := httptest.NewRecorder()
			handler.ListSemanticAttributeDefinitions(response, request)
			if response.Code != http.StatusBadRequest {
				t.Fatalf("invalid search status=%d body=%s", response.Code, response.Body.String())
			}
		})
	}
}

func TestSemanticAttributeHTTPUpdateCompletesGeneratedConcurrencyContract(t *testing.T) {
	repo := semanticAttributeHTTPRepository()
	handler := semanticAttributeHTTPHandler(repo, true)
	request := semanticAttributeRequest(http.MethodPatch, "/api/v1/semantic-attributes/region", `{"ownerKind":"instance","displayName":"Region","description":"Updated"}`, "attribute", "region")
	contract, ok := accessgen.GetAPIGenCommandRuntimeContract("updateSemanticAttributeMetadata")
	if !ok {
		t.Fatal("updateSemanticAttributeMetadata command contract is missing")
	}
	currentETag := semanticAttributeDefinitionETag(repo.definition)
	ctx, guard, err := apigencommand.BeginInvocation(request.Context(), contract, apigencommand.Invocation{
		Surface: apigencommand.SurfaceAPI, TargetValues: map[string]string{"attribute": repo.definition.Name},
		ConcurrencyToken: currentETag,
	})
	if err != nil {
		t.Fatal(err)
	}
	request = request.WithContext(ctx)
	request.Header.Set("If-Match", currentETag)
	response := httptest.NewRecorder()
	handler.UpdateSemanticAttributeMetadata(response, request)
	if response.Code != http.StatusOK {
		t.Fatalf("generated update status=%d body=%s", response.Code, response.Body.String())
	}
	if !guard.Completed() {
		t.Fatal("generated updateSemanticAttributeMetadata command did not complete its concurrency and transactional contract")
	}
}
...
func TestSemanticAttributeHTTPClaimMappingRequiresCompleteTrustIdentity(t *testing.T) {
	...
	for _, body := range []string{
		...
		`{"sourceKind":"oidc","provider":" corp","issuer":"https://issuer","audience":"aud","claim":"department"}`,
		`{"sourceKind":"oidc","provider":"corp","issuer":"https://issuer\n","audience":"aud","claim":"department"}`,
		`{"sourceKind":"oidc","provider":"corp","issuer":"https://issuer","audience":"aud","claim":" department"}`,
		`{"sourceKind":"oidc","provider":"corp","issuer":"https://issuer","audience":"aud","claim":"department\n"}`,
	} {
