...
// ValidateClaimName validates the exact top-level claim identifier accepted by
// the trusted-claim boundary. Admission APIs use the same validator as the
// verifier so malformed mapping identities fail as client errors before any
// repository lookup or mutation.
func ValidateClaimName(name string) error {
	return validateClaimName(name)
}
