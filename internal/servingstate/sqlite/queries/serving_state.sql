...
UPDATE serving_states
SET status = ?, project_id = ?, project_digest = ?, project_workspaces_json = ?, access_policy_json = ?, dashboard_publications_json = ?, dashboard_appearances_json = ?, digest = ?, manifest_json = ?, error = ''
WHERE id = ?;
