...
func seedCommittedDeliveryPublication(t *testing.T, store *platform.Store, generationID string) {
	...
	statements := []struct {
		...
		{`INSERT INTO physical_pools (id, identity_digest, storage_location, storage_namespace, storage_implementation, object_naming_contract, encryption_domain, isolation_boundary, retention_authority, retention_policy_json) VALUES (?, ?, 's3://versions-test', 'versions-test', 's3', 'names-v1', 'versions-test', 'versions-test', 'gc', '{}')`, []any{digest("9"), digest("9")}},
