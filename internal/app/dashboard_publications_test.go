...
func TestDashboardPublicationManagementAPIRequiresAndReplaysIdempotencyKeys(t *testing.T) {
	...
	listRequest := httptest.NewRequest(http.MethodGet, "/api/v1/dashboard-publications", nil)
	...
	if strings.Contains(list.Body.String(), `"projectId"`) {
		t.Fatalf("list response exposed internal project identity: %s", list.Body.String())
	}

	path := "/api/v1/dashboard-publications/website/suspend"
