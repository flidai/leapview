...
func bootstrapQualificationRoleBindings(
	...
	for _, item := range bindings {
		body, err := json.Marshal(map[string]any{
			...
			"role": string(access.ProjectRoleAdmin), "expectedRevision": item.expectedRevision,
		})
		if err != nil {
			return 0, "", err
		}
		...
		if created.ID != item.id || created.Name != item.name || created.SubjectType != "principal" || created.SubjectID != item.subjectID || created.Role != string(access.ProjectRoleAdmin) ||
			created.PolicyRevision != item.expectedRevision+1 || len(created.Capabilities) != len(wantCapabilities) {
	...
	for _, created := range policy.Items {
		...
		if created.Name != item.name || created.SubjectType != "principal" || created.SubjectID != item.subjectID || created.Role != string(access.ProjectRoleAdmin) ||
			created.PolicyRevision != policy.PolicyRevision || created.PolicyDigest != policy.PolicyDigest || len(created.Capabilities) != len(wantCapabilities) {
		...
		canonicalBindings = append(canonicalBindings, access.RoleBinding{
			ID: created.ID, Name: created.Name,
			Subject: access.SubjectRef{Kind: access.SubjectKindPrincipal, ID: created.SubjectID},
			Role:    access.ProjectRoleAdmin, Capabilities: capabilities,
		})
	}
