...
func stageDeclaredDevelopmentInputs(ctx context.Context, credentials cliapi.Credentials, local localDevelopmentSession) (string, error) {
...
func stageDeclaredDevelopmentInputsWithDependencies(
	...
	hash := sha256.New()
	for _, input := range planned {
		revision := input.plan.Manifest.RevisionID()
		_, _ = fmt.Fprintf(hash, "%d:%s:%d:%s:%d:%s:", len(input.name), input.name, len(input.selection.Connection), input.selection.Connection, len(revision), revision)
	}
