...
func TestBootstrapReplayVerifiesResourceGrantsAtSamePolicyRevision(t *testing.T) {
	...
	owner := access.RoleBinding{ID: bootstrapOwnerBindingID, Name: bootstrapOwnerBindingName, Subject: access.SubjectRef{Kind: access.SubjectKindPrincipal, ID: principal}, Role: access.ProjectRoleAdmin, Capabilities: access.ProjectRoleCapabilities(access.ProjectRoleAdmin)}
	...
	digest, err := access.AuthorizationPolicyDigest(scope, []access.RoleBinding{owner}, grant)
	if err != nil {
		t.Fatal(err)
	}
		...
		t.Run(mode, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method == http.MethodPost {
					serveBootstrapOwnerBinding(t, w, r, target, project, environment, principal)
					return
				}
				response := map[string]any{"targetId": target, "projectId": project, "environment": environment, "policyRevision": 2, "policyDigest": digest, "page": map[string]any{}}
				if strings.HasSuffix(r.URL.Path, "/grants") {
					grantReads++
					item := map[string]any{"id": grant.ID, "name": grant.Name, "subjectType": "principal", "subjectId": grant.Subject.ID, "resourceId": resource.ID(), "resourceKind": resource.Kind(), "capability": grant.Capability, "policyRevision": 2, "policyDigest": digest}
					response["items"] = []any{item}
					switch mode {
					case "stale":
						response["policyRevision"] = 3
					case "omitted":
						response["items"] = []any{}
					case "tampered":
						item["name"] = "different"
					...
					response["items"] = []any{map[string]any{"id": owner.ID, "name": owner.Name, "subjectType": "principal", "subjectId": principal, "role": "admin", "capabilities": owner.Capabilities, "policyRevision": 2, "policyDigest": digest}}
				}
			...
			if mode == "valid" {
				if err != nil || revision != 2 || actual != digest {
