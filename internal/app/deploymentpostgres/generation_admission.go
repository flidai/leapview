...
type GenerationAdmissionInput struct {
	...
	// legacyAuthorizationPolicy is set only by the recovery assembler for a
	// pre-017 plan whose immutable seal has no target-policy revision columns.
	// Fresh admission cannot opt into this compatibility path.
...
func (a *generationAdmitter) CompleteBuildAndAdmitTx(ctx context.Context, tx deploymentnative.Tx, input GenerationAdmissionInput) (GenerationAdmissionResult, error) {
	...
	if normalized.legacyAuthorizationPolicy {
		if err := validateLegacyGenerationAuthorizationSnapshot(normalized); err != nil {
			return GenerationAdmissionResult{}, err
		}
	} else {
	...
	}, toNativeSeal(normalized), normalized.QualificationDigest, toNativeFence(normalized.Fence))
	if err != nil {
		return GenerationAdmissionResult{}, err
	}
	...
	if normalized.legacyAuthorizationPolicy {
		// Version four predates target-policy revision metadata. Its immutable
		// policy digest remains in PolicyDigest; the compiled fingerprint remains
		// bound by the plan, seal, generation, and serving policy checks above.
		provenanceInput.Plan.AuthorizationDigest = ""
		provenance, err = release.NewLegacyProvenance(provenanceInput)
	} else {
...
func validateAdmissionProvenanceInput(input release.ProvenanceInput, admission GenerationAdmissionInput) error {
	...
	if admission.legacyAuthorizationPolicy {
		if input.Plan.PolicyRevision != 0 || admission.AuthorizationPolicy.Revision != 0 || admission.AuthorizationPolicy.Digest != "" {
			return fmt.Errorf("%w: legacy release provenance contains target authorization policy evidence", deploymentnative.ErrConflict)
		}
	} else if input.Plan.PolicyRevision != admission.AuthorizationPolicy.Revision || input.Plan.PolicyDigest != admission.AuthorizationPolicy.Digest {
		return fmt.Errorf("%w: release provenance plan identity differs", deploymentnative.ErrConflict)
	}
	return nil
}
...
func (a *generationAdmitter) authoritativeCandidateExpiry(ctx context.Context, tx deploymentnative.Tx, input GenerationAdmissionInput) (time.Time, error) {
	...
	if plan.Governance.AuthorizationDigest != input.Generation.SecurityDomainFingerprint {
	...
	if input.legacyAuthorizationPolicy {
		if plan.Governance.PolicyRevision != 0 || input.AuthorizationPolicy.Revision != 0 || input.AuthorizationPolicy.Digest != "" || plan.Governance.PolicyDigest != input.Provenance.Plan.PolicyDigest {
			return time.Time{}, fmt.Errorf("%w: persisted legacy delivery plan authorization policy differs from generation admission", deploymentnative.ErrConflict)
		}
	} else if plan.Governance.PolicyRevision != input.AuthorizationPolicy.Revision || plan.Governance.PolicyDigest != input.AuthorizationPolicy.Digest {
		return time.Time{}, fmt.Errorf("%w: persisted delivery plan authorization policy differs from generation admission", deploymentnative.ErrConflict)
	}
...
func normalizeInput(input GenerationAdmissionInput) (GenerationAdmissionInput, error) {
	...
	if ctx.legacyAuthorizationPolicy {
		if ctx.AuthorizationPolicy.Revision != 0 || ctx.AuthorizationPolicy.Digest != "" || ctx.Seal.AuthorizationPolicyRevision != 0 || ctx.Seal.AuthorizationPolicyDigest != "" {
			return GenerationAdmissionInput{}, conflict("legacy snapshot seal contains target authorization policy evidence")
		}
	} else {
