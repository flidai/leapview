...
func nativePhysicalEnvironment(t *testing.T, in NativePhysicalBuildInput) *nativePhysicalEnvironmentFake {
	...
	snapshot := int64(42)
	relations := []ducklake.BaseTable{{Schema: in.Attempt.Namespace, Table: "orders"}}
	...
	}{in.Attempt.Namespace, relations})
	if err != nil {
		t.Fatal(err)
	}
	...
	}{2, in.CatalogID, snapshot, root, in.Attempt.Namespace, relations, []ducklake.NativeSnapshotObject{}, relationDigest, closureDigest, rootDigest})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildNativePhysicalRejectsMismatchedEnvironmentCatalogIdentityAndCloses(t *testing.T) {
	in := nativePhysicalFixtureInput(t)
	env := nativePhysicalEnvironment(t, in)
	env.closure.CatalogID = "other-catalog"
	...
	if err == nil || !errors.Is(err, deploymentnative.ErrConflict) {
		t.Fatalf("catalog identity error = %v, want conflict", err)
	}
	if env.closes != 1 {
