...
func TestAuthorizationRoleBindingsFromServingPolicyRejectsAmbiguousEvidence(t *testing.T) {
...
func TestInitializeActiveTargetAuthorizationPolicyDefersValidLegacyStructures(t *testing.T) {
	tests := map[string]string{
		"groups":                       `{"groups":{"group-sales":{"id":"group-sales","name":"Sales"}}}`,
		"grants":                       `{"grants":{"grant-dashboard":{"id":"grant-dashboard","name":"Dashboard reader","object":{"id":"dashboard-main","kind":"dashboard"},"subject":{"kind":"principal","principalId":"70000000-0000-0000-0000-000000000001"},"capability":"RESOURCE_READ"}}}`,
		"data policies":                `{"dataPolicies":{"policy-dashboard":{"id":"policy-dashboard","name":"Region filter","object":{"id":"dashboard-main","kind":"dashboard"},"policyType":"row_filter","expressionJson":"{\"field\":\"region\",\"value\":\"emea\"}"}}}`,
		"role binding name whitespace": `{"roleBindings":{"binding-owner":{"id":"binding-owner","name":" Existing owner ","role":"owner","subject":{"kind":"principal","principalId":"70000000-0000-0000-0000-000000000001"}}}}`,
	}
	for name, policyJSON := range tests {
		t.Run(name, func(t *testing.T) {
			db := authorizationPolicyUpgradeDB(t)
			seedAuthorizationPolicyUpgradeFixtureWithPolicy(t, db, policyJSON)
			targets := deploymentpostgres.New(db)
			states := servingstatepostgres.New(db)
			scope := access.AuthorizationPolicyScope{TargetID: authorizationPolicyUpgradeTargetID, ProjectID: authorizationPolicyUpgradeProjectID, Environment: authorizationPolicyUpgradeEnvironment}
			policies, err := accesspostgres.NewAuthorizationPolicyRepository(db, scope)
			...
			begin := func(ctx context.Context) (accesspostgres.Tx, error) { return db.Begin(ctx) }
			if err := appaccesspostgres.InitializeActiveTargetAuthorizationPolicy(t.Context(), begin, targets, states, policies, scope.TargetID, scope.Environment); err != nil {
				t.Fatalf("startup authorization policy initialization: %v", err)
			}
			if _, err := policies.AuthorizationPolicy(t.Context(), scope); !errors.Is(err, access.ErrAuthorizationPolicyNotFound) {
				t.Fatalf("deferred legacy policy created target state: %v", err)
			}
			var retained string
			if err := db.QueryRow(t.Context(), `SELECT access_policy_json::text FROM serving_state.bundle WHERE generation_id=$1::uuid`, authorizationPolicyUpgradeGenerationID).Scan(&retained); err != nil {
				t.Fatal(err)
			}
			var got, want any
			if err := json.Unmarshal([]byte(retained), &got); err != nil {
				t.Fatal(err)
			}
			if err := json.Unmarshal([]byte(policyJSON), &want); err != nil {
				t.Fatal(err)
			}
			if !reflect.DeepEqual(got, want) {
				t.Fatalf("active legacy policy changed: got %s want %s", retained, policyJSON)
			}
		})
	}
}
