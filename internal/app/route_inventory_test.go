...
func TestRouteInventory(t *testing.T) {
	...
	if got["POST /api/v1/instance/project-claim"] != 1 {
		t.Fatalf("project claim bootstrap route registered %d times, want exactly once", got["POST /api/v1/instance/project-claim"])
	}
	...
	sort.Strings(rows)
	const expectedRouteContractDigest = "f90a651357df8e0fdb62a59e66e4cd6d0c292b91d788f6cc19a0dd5c5861752f"
...
func nonAPIRouteMetadata(method, path string) (routeMetadata, bool) {
	...
	case path == "/dashboards/{dashboard}/edit" || path == "/dashboards/{dashboard}/draft/command" || path == "/dashboards/{dashboard}/draft/visual-window":
		authenticated.owner = "dashboard"
		authenticated.privilege = "RESOURCE_EDIT"
	case path == "/dashboards/{dashboard}/delete":
		authenticated.owner = "dashboard"
	...
	case path == "/" || path == "/search" || path == "/catalog/search" || path == "/sources" || strings.HasPrefix(path, "/sources/") ||
		...
		path == "/pipelines" || strings.HasPrefix(path, "/pipelines/") || path == "/runs" || path == "/connections" || strings.HasPrefix(path, "/connections/"):
		authenticated.owner = "project"
		authenticated.privilege = "RESOURCE_READ"
	case path == "/updates":
