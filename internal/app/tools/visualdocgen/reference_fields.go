...
var presentationFieldReferences = map[string]visualdocs.FieldReference{
	...
	"axes":                  field("axis configuration list", "renderer-neutral defaults", nil, "Configures governed Cartesian axes, domains, units, zero policy, and tick density."),
	"referenceLines":        field("reference line list", "none", nil, "Adds governed constant or field-reduced reference markers on supported axes."),
	"referenceBands":        field("reference band list", "none", nil, "Adds governed intervals with explicit semantic tones on supported axes."),
	"eventAnnotations":      field("event annotation list", "none", nil, "Adds governed labeled events on the horizontal axis."),
