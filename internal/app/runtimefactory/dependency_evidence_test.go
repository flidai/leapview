...
func TestDependencyEvidenceRelationProjectionIgnoresPresentationAndRotatesExecution(t *testing.T) {
	...
	dependencyFor := func(manifest projectmanifest.Project) resultidentity.Dependency {
		t.Helper()
		artifact, err := projectartifact.NewProject(graphValue, manifest)
		if err != nil {
			t.Fatal(err)
	...
	base := dependencyFor(baseManifest)
	...
	if got := dependencyFor(presentation); got.Digest() != base.Digest() {
	...
	if got := dependencyFor(execution); got.Digest() == base.Digest() {
