...
func TestAPIGenAgentCapabilityOwnsItsOperationSurface(t *testing.T) {
	...
	if got, want := len(apiaggregate.GetAPIGenOperationContracts()), 185; got != want {
...
func TestAPIGenIRAssignsCapabilityNamespaces(t *testing.T) {
	...
	if got, want := len(document.Endpoints), 185; got != want {
...
func TestAPIGenOwnsUISignalContracts(t *testing.T) {
	...
	if len(irDoc.Contracts) != 115 {
		t.Fatalf("UI signal IR contracts = %d, want 115", len(irDoc.Contracts))
	}
