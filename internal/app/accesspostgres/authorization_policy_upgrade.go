...
func InitializeActiveTargetAuthorizationPolicy(
	...
	if !migratable {
		// The active generation remains authoritative for policy structures that
		// the role-binding-only target policy cannot preserve. Defer initialization
		// instead of dropping valid authorization state or blocking startup.
		return nil
	}
	if len(bindings) == 0 {
