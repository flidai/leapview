...
// dashboardPublicationServingStateReader is the read-only serving-state
// authority needed by activation reconciliation.  It is deliberately kept
// independent of the storage dialect so the native and legacy paths share
// the same generation and stale-callback checks.
type dashboardPublicationServingStateReader interface {
	ByID(context.Context, servingstate.ID) (servingstate.State, error)
}

// dashboardPublicationActivationReconciler is injected into runtime
// composition.  Keeping the callback behind this capability boundary means a
// production router never has to discover a database or select SQLite based
// on a nil optional field.
type dashboardPublicationActivationReconciler interface {
	Reconcile(context.Context, dashboardPublicationServingStateReader, deployment.Deployment) error
}
...
func (r *NativeDashboardPublicationReconciler) Reconcile(ctx context.Context, states dashboardPublicationServingStateReader, activated deployment.Deployment) error {
...
// ownership.
func (r *NativeDashboardPublicationReconciler) ReconcileTx(ctx context.Context, tx pgx.Tx, states dashboardPublicationServingStateReader, activated deployment.Deployment) error {
...
func loadActivatedDashboardPublications(ctx context.Context, states dashboardPublicationServingStateReader, activated deployment.Deployment) (servingstate.State, map[string]json.RawMessage, bool, error) {
	if states == nil {
