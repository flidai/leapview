...
func assemblePostgresPublicJourneyRoutes(t *testing.T, fixture *PostgresJourneyFixture, runtime *runtimehostmodule.Module, accessSurface *accessmodule.Module, auth *accessmodule.Auth) http.Handler {
	...
	workflow := workflowAssemblyInputs{Workload: fixture.Workload, AgentSettings: fixture.Graph.Bootstrap, Auth: auth}
	...
	routes, runtimeServices, platform, policy, err := buildApplicationSurfaces(t.Context(), fakeMetrics{}, data, capabilities, workflow, runtimeConfig, httpAssemblyInputs{PublicURL: "http://localhost"})
	if err != nil {
