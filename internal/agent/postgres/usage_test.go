...
func TestPostgreSQL18ModelRequestUsageReserveIsAtomicAndRollsForward(t *testing.T) {
	pool, repo := agentPostgresTestRepo(t, "model_request_usage")
	ctx := t.Context()
	...
	if _, err := pool.Exec(ctx, `
        UPDATE agent.model_request_usage
        SET usage_day = (CURRENT_TIMESTAMP AT TIME ZONE 'UTC')::date - 1,
            used_requests = $1
	...
	var persisted int64
	if err := pool.QueryRow(ctx, `SELECT used_requests FROM agent.model_request_usage WHERE singleton_id = 1`).Scan(&persisted); err != nil {
