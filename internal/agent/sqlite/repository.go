...
var pendingConversationMetadataSQLite = `
UPDATE agent_conversations
SET metadata_json = ?, updated_at = CURRENT_TIMESTAMP
WHERE id = ? AND principal_id = ? AND status IN ('active', 'archived')
...
func (r *Repository) BeginPendingConversationAction(ctx context.Context, pending agent.PendingConversationAction) (agent.Conversation, error) {
	...
	err = r.withConversationMutationTx(ctx, func(tx *sql.Tx, q *platformdb.Queries) error {
		...
		result, err := tx.ExecContext(ctx, pendingConversationMetadataSQLite, metadata, pending.ConversationID, principalID)
		if err != nil {
			return err
		}
...
func (r *Repository) CancelPendingConversationAction(ctx context.Context, principalID, conversationID, requestID string) error {
	...
	return r.withConversationMutationTx(ctx, func(tx *sql.Tx, q *platformdb.Queries) error {
		...
		result, err := tx.ExecContext(ctx, pendingConversationMetadataSQLite, metadata, conversationID, principalID)
		if err != nil {
			return err
		}
		if affected, _ := result.RowsAffected(); affected != 1 {
...
func (r *Repository) ListPendingConversationActions(ctx context.Context) ([]agent.PendingConversationAction, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT principal_id, id, json_extract(metadata_json, '$._leapview_chat.pendingAction'), json_extract(metadata_json, '$._leapview_chat.pendingRequestId'), json_extract(metadata_json, '$._leapview_chat.pendingUntil') FROM agent_conversations WHERE json_extract(metadata_json, '$._leapview_chat.pendingAction') IN ('archive', 'delete')`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
