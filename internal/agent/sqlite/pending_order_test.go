...
func TestPendingUndoPreservesHistoryOrder(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			owner := createAgentPrincipal(t, ctx, store, "undo-order@example.com").ID
			for i := range 3 {
				...
				if _, err := store.SQLDB().ExecContext(ctx, `UPDATE agent_conversations SET updated_at = ? WHERE id = ?`, stamp, conversation.ID); err != nil {
					t.Fatal(err)
				}
			}
