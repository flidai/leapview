...
// message row. Model IDs are generated by pkg/agent, while UI edit commands
// carry the persisted agent_messages ID, so the ordered mapping is required
// when resolving an edit target.
type activeTranscriptEntry struct {
	core      agentcore.Message
	coreIndex int
	stored    Message
	storedOK  bool
}

func prepareEditedTranscript(active []agentcore.Message, stored []Message, targetID string) ([]agentcore.Message, error) {
	targetID = strings.TrimSpace(targetID)
	entries := activeTranscriptEntries(active, stored)
	targetIndex := -1
	for _, entry := range entries {
		if entry.core.Role != agentcore.RoleUser || entry.core.Kind == agentcore.MessageKindExternalContext {
			continue
		}
		// The transport exposes persisted message IDs. Core model IDs are
		// deliberately not accepted here because they cannot anchor the
		// immutable-row active projection after the edit is stored.
		if entry.storedOK && entry.stored.ID == targetID {
			targetIndex = entry.coreIndex
			break
		}
	}
	if targetIndex < 0 {
		return nil, fmt.Errorf("edit target %q is not an active user message", targetID)
	}
	// Context messages immediately before a user message belong to that turn;
	// remove them with the replaced question so the replacement can carry fresh
	// server-resolved context.
	for targetIndex > 0 && active[targetIndex-1].Kind == agentcore.MessageKindExternalContext {
		targetIndex--
	}
	trimmed := make([]agentcore.Message, targetIndex)
	copy(trimmed, active[:targetIndex])
	return trimmed, nil
}

func activeTranscriptEntries(active []agentcore.Message, stored []Message) []activeTranscriptEntry {
	// Rows from replaced branches remain immutable. Use the same active-row
	// projection as reloads before matching legacy rows that predate the
	// message_id metadata, otherwise identical text could bind to an old row.
	stored = activeMessageProjection(stored)
	entries := make([]activeTranscriptEntry, 0, len(active))
	storedIndex := 0
	for index, core := range active {
		if core.Role == agentcore.RoleSystem || core.Role == agentcore.RoleSummary || core.Kind == agentcore.MessageKindExternalContext {
			continue
		}
		entry := activeTranscriptEntry{core: core, coreIndex: index}
		for candidate := storedIndex; candidate < len(stored); candidate++ {
			storedCoreID := storedMessageCoreID(stored[candidate].ContentJSON)
			if storedCoreID != "" && core.ID != "" && storedCoreID != core.ID {
				continue
			}
			if stored[candidate].Role == platformRole(core.Role) && stored[candidate].ContentText == activeMessageText(core) && activeMessageMatchesStored(core, stored[candidate]) {
				entry.stored = stored[candidate]
				entry.storedOK = true
				storedIndex = candidate + 1
				break
			}
		}
...
func storedMessageCoreID(raw string) string {
	var payload struct {
...
func activeMessageMatchesStored(core agentcore.Message, stored Message) bool {
	if core.Role != agentcore.RoleTool {
		return true
	}
	return strings.TrimSpace(core.ToolCallID) == "" || strings.TrimSpace(core.ToolCallID) == strings.TrimSpace(stored.ToolCallID)
}

func activeMessageProjection(messages []Message) []Message {
	projected := make([]Message, 0, len(messages))
	for _, message := range messages {
		if message.Role == MessageRoleUser {
			if targetID := editTargetFromContentJSON(message.ContentJSON); targetID != "" {
				for index := range projected {
					if projected[index].ID == targetID && projected[index].Role == MessageRoleUser {
...
func editTargetFromContentJSON(raw string) string {
	var payload struct {
	...
	return strings.TrimSpace(payload.EditMessageID)
}
	...
	for _, message := range messages {
		...
		case MessageRoleUser:
			items = append(items, ChatTranscriptItem{
				ID:             message.ID,
				Kind:           "user",
				Edited:         editTargetFromContentJSON(message.ContentJSON) != "",
