...
func TestEmit_ComposesTypedCapabilityServers(t *testing.T) {
	...
	require.Contains(t, content, "func GetAPIGenUIActions() []apigenui.Action")
	require.Contains(t, content, "for _, action := range accessapi.GetAPIGenUIActions()")
	require.Contains(t, content, "for _, action := range dashboardapi.GetAPIGenUIActions()")
	...
	require.Contains(t, content, "type GenAuthzContract struct {")
	require.Contains(t, content, "Authz *GenAuthzContract")
	...
	require.Contains(t, content, "if contract.Authz != nil { source := contract.Authz; authz = &GenAuthzContract{")
