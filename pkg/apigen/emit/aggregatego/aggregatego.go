...
func emitProtocolMetadata(b *strings.Builder, packages []resolvedServerPackage, embeddedOpenAPI string) {
	...
	b.WriteString("type GenAuditField struct { Name string; Sensitivity string }\n\n")
	b.WriteString("type GenAuditPayloadContract struct { Schema string; SchemaVersion int; Retention string; Fields []GenAuditField }\n\n")
	b.WriteString("type GenAuditPolicy struct { Required bool; SuccessAction string; Guarantee string; Payload *GenAuditPayloadContract }\n\n")
	b.WriteString("type GenAsyncExecutionContract struct { Mode string; Guarantee string; JobKind string; ResourceKind string; InitialEvent string; InitialState string; StatusOperation string; EventsOperation string; Cancellation string }\n\n")
	b.WriteString("type GenCommandFailure struct { Kind string; StatusCode int; Code string; PublicDetail string }\n\n")
	b.WriteString("type GenOperationTarget struct { Parameter string; Type string }\n\n")
	b.WriteString("type GenUIActionContract struct { ActionID string }\n\n")
	b.WriteString("// GenAuthzContract captures the operation authorization annotation.\n")
	b.WriteString("type GenAuthzContract struct { Mode string; Privilege string; Action string; Resolver string }\n\n")
	...
	for _, serverPackage := range packages {
		...
		b.WriteString("\t\tif contract.Command != nil { command = &GenCommandContract{Owner: contract.Command.Owner, Audit: GenAuditPolicy{Required: contract.Command.Audit.Required, SuccessAction: contract.Command.Audit.SuccessAction, Guarantee: contract.Command.Audit.Guarantee}, Failures: make([]GenCommandFailure, len(contract.Command.Failures)), AdditionalExposures: make([]GenOperationSurface, len(contract.Command.AdditionalExposures)), Idempotency: contract.Command.Idempotency, Concurrency: contract.Command.Concurrency, AuthzMode: contract.Command.AuthzMode, Privilege: contract.Command.Privilege}\n")
		b.WriteString("\t\t\tif contract.Command.Audit.Payload != nil { source := contract.Command.Audit.Payload; command.Audit.Payload = &GenAuditPayloadContract{Schema: source.Schema, SchemaVersion: source.SchemaVersion, Retention: source.Retention, Fields: make([]GenAuditField, len(source.Fields))}; for index, field := range source.Fields { command.Audit.Payload.Fields[index] = GenAuditField{Name: field.Name, Sensitivity: field.Sensitivity} } }\n")
		b.WriteString("\t\t\tfor index, failure := range contract.Command.Failures { command.Failures[index] = GenCommandFailure{Kind: failure.Kind, StatusCode: failure.StatusCode, Code: failure.Code, PublicDetail: failure.PublicDetail} }\n")
		b.WriteString("\t\t\tfor index, exposure := range contract.Command.AdditionalExposures { command.AdditionalExposures[index] = GenOperationSurface(exposure) }\n")
		b.WriteString("\t\t\tif contract.Command.UI != nil { command.UI = &GenUIActionContract{ActionID: contract.Command.UI.ActionID} }\n")
		b.WriteString("\t\t\tif contract.Command.Target != nil { command.Target = &GenOperationTarget{Parameter: contract.Command.Target.Parameter, Type: contract.Command.Target.Type} }\n")
		b.WriteString("\t\t\tif contract.Command.Execution != nil { source := contract.Command.Execution; command.Execution = &GenAsyncExecutionContract{Mode: source.Mode, Guarantee: source.Guarantee, JobKind: source.JobKind, ResourceKind: source.ResourceKind, InitialEvent: source.InitialEvent, InitialState: source.InitialState, StatusOperation: source.StatusOperation, EventsOperation: source.EventsOperation, Cancellation: source.Cancellation} }\n")
		b.WriteString("\t\t}\n")
		b.WriteString("\t\tvar authz *GenAuthzContract\n")
		b.WriteString("\t\tif contract.Authz != nil { source := contract.Authz; authz = &GenAuthzContract{Mode: source.Mode, Privilege: source.Privilege, Action: source.Action, Resolver: source.Resolver} }\n")
		...
		b.WriteString("\t\t\tRequestBodyRequired: contract.RequestBodyRequired, AuthzMode: contract.AuthzMode, Authz: authz,\n")
	...
	b.WriteString("var genAPIGenUIActions = func() []apigenui.Action {\n")
	b.WriteString("\tvar out []apigenui.Action; actions := map[string]string{}; operations := map[string]string{}\n")
	for _, serverPackage := range packages {
		fmt.Fprintf(b, "\tfor _, action := range %s.GetAPIGenUIActions() {\n", serverPackage.Alias)
		b.WriteString("\t\tif operationID, exists := actions[action.ActionID()]; exists { panic(\"duplicate aggregate APIGen UI action \" + action.ActionID() + \" for \" + operationID + \" and \" + action.OperationID()) }; if actionID, exists := operations[action.OperationID()]; exists { panic(\"duplicate aggregate APIGen UI operation \" + action.OperationID() + \" for \" + actionID + \" and \" + action.ActionID()) }; actions[action.ActionID()] = action.OperationID(); operations[action.OperationID()] = action.ActionID(); out = append(out, action)\n")
		b.WriteString("\t}\n")
	}
	b.WriteString("\treturn out\n}()\n\n")
	b.WriteString("// GetAPIGenUIActions returns every generated aggregate UI action binding.\n")
	b.WriteString("func GetAPIGenUIActions() []apigenui.Action { return append([]apigenui.Action(nil), genAPIGenUIActions...) }\n\n")
	...
	b.WriteString("}\n\n")
	b.WriteString("// GetAPIGenCommandRuntimeContracts returns every normalized aggregate command policy.\n")
	...
	b.WriteString("}\n\n")
	b.WriteString("// GetAPIGenOperationContractForRequest resolves aggregate generated policy from a concrete method and path.\n")
	...
	b.WriteString("}\n\n")
	b.WriteString("// GetAPIGenCommandFailureContracts returns the aggregate domain-failure vocabulary for a command.\n")
