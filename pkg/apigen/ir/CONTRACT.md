...
The endpoint `x-authz` extension requires a string `mode` and may carry the legacy string `privilege`. It may additionally declare an opaque typed `action` (a stable dotted lower-snake-case name) and a named `resolver` (a stable lower-case dotted domain dependency). APIGen validates only these names; product authorization packages own the action vocabulary and resolver behavior. Generated operation contracts expose the complete annotation through `GenAuthzContract`.
