...
`SchemaRef.additional_properties` represents typed maps. A nested schema preserves `Record<T>` recursively across Go, TypeScript, OpenAPI, JSON Schema, and agent-tool output. Numeric `const` values and authored `min_items`, `max_items`, and `min_properties` constraints remain available to canonical schema emitters and runtime bindings. Model-visible agent input schemas are a provider-portable projection: numeric constants become single-value `enum` values, while unsupported cardinality keywords are omitted. These optional constraints extend IR v4 without changing the meaning of existing v4 documents. Integer `format` is authoritative: `int32` maps to Go `int32`, while `int64` maps to Go `int64` without narrowing.
