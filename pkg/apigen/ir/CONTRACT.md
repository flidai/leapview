...
`SchemaRef.additional_properties` represents typed maps. A nested schema preserves `Record<T>` recursively across Go, TypeScript, OpenAPI, JSON Schema, and agent-tool output. Numeric `const` values preserve TypeSpec numeric literals, while `min_items` and `max_items` preserve authored array cardinality constraints across those same emitters. These optional constraints extend IR v4 without changing the meaning of existing v4 documents. Integer `format` is authoritative: `int32` maps to Go `int32`, while `int64` maps to Go `int64` without narrowing.
