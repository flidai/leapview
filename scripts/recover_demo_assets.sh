...
if curl -fsS --connect-timeout 2 --max-time 5 https://demo.leapview.dev/readyz >/dev/null 2>&1; then
  ...
  ui_revision=a3a0b4e34a5d9e83109833fbab44fd4831c64221
  ui_marker="$repo/.tmp/leapview-dev.agent-lease-v6"
  if [[ "$(cat "$ui_marker" 2>/dev/null || true)" != "$ui_revision" ]]; then
    ...
    ui_binary="$repo/.tmp/leapview-dev.agent-lease-v6"
...
ui_revision=a3a0b4e34a5d9e83109833fbab44fd4831c64221
delivery_hotfix_marker="$repo/.tmp/leapview-dev.agent-lease-v10"
hotfix_binary="$repo/.tmp/leapview-dev.agent-lease-v10"
