...
while IFS= read -r database_container; do
  ...
  docker exec "$database_container" sh -c 'psql -U "$POSTGRES_USER" -d leapview_control -Atc "SELECT datname, pg_size_pretty(pg_database_size(oid)) FROM pg_database WHERE datname IN ('leapview_control','leapview_ducklake')"'
