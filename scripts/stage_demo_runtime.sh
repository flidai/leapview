...
image=ghcr.io/flidai/leapview@sha256:83ef54b4832144b6c8f01c39ca220bb86c51785ac20e5f590099cede8f83ee9f
release=/opt/leapview-demo/releases/$revision
service=leapview-demo-current.service
if ! systemctl is-active --quiet "$service"; then
  preserved_revision=f29a0c9a88cb57f5ce0544fa0b5bb3d29c28f883
...
if [[ -x "$release/leapview" && -f "$release/immutable-image.txt" && -f "$release/leapview.sha256" ]]; then
  ...
  printf 'Exact qualified main image already staged at %s; active service unchanged\n' "$release"
  exit 0
fi
...
printf 'Staged exact qualified main image at %s; active service unchanged\n' "$release"
REMOTE
