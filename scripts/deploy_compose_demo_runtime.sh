...
trap 'status=$?; echo "Compose demo deployment failed at line $LINENO (status $status)" >&2' ERR
...
ssh -i "$identity_file" -o BatchMode=yes -o ConnectTimeout=10 \
  ...
  "root@$demo_host" 'python3 -' \
  < "$repo_root/scripts/deploy_compose_demo_runtime.py"
