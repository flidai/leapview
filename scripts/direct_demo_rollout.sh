...
predecessor_schema="${DIRECT_DEMO_PREDECESSOR_SCHEMA:-22}"
expected_schema="${DIRECT_DEMO_EXPECTED_SCHEMA:-23}"
rollout_action="${DIRECT_DEMO_ROLLOUT_ACTION:?Set DIRECT_DEMO_ROLLOUT_ACTION to prepare or deploy}"
public_url="${DIRECT_DEMO_PUBLIC_URL:-https://demo.leapview.dev}"
...
[[ "$public_url" =~ ^https://[A-Za-z0-9.-]+$ ]] || { echo 'DIRECT_DEMO_PUBLIC_URL must be an HTTPS origin' >&2; exit 64; }
case "$rollout_action" in prepare|deploy) ;; *) echo 'DIRECT_DEMO_ROLLOUT_ACTION must be prepare or deploy' >&2; exit 64 ;; esac
phase='--check'
[[ "$rollout_action" == deploy ]] && phase='--apply'
...
ssh -i "$identity_file" -o BatchMode=yes -o ConnectTimeout=10 \
  ...
  "root@$demo_host" "DIRECT_DEMO_REVISION=$revision DIRECT_DEMO_IMAGE=$image DIRECT_DEMO_PREDECESSOR_SCHEMA=$predecessor_schema DIRECT_DEMO_EXPECTED_SCHEMA=$expected_schema DIRECT_DEMO_PUBLIC_URL=$public_url python3 - $phase" < "$repo_root/scripts/direct_demo_rollout.py"
