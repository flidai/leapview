...
export async function verifyDataExplorerRecoveryActions({
  ...
  try {
    ...
    const semanticResetCommand = JSON.parse(semanticResetBody).dataExplorerCommand?.explore
    if (!Number.isInteger(semanticResetCommand?.resetVersion)
      || semanticResetCommand?.action !== 'configure') {
...
async function enterDataExplorerAnalyzeMode(page: Page): Promise<{ explorer: Locator; controls: Locator }> {
  ...
  if (await analyze.count() === 0) {
    ...
    const firstObject = firstGroup.locator('.object-button').first()
    await firstObject.waitFor({ state: 'visible' })
    const selectionRequest = page.waitForRequest((request) => new URL(request.url()).pathname === '/explore/command' && request.method() === 'POST')
    await firstObject.click()
...
export async function verifyDataExplorerKeyboardJourney({
  ...
  try {
    ...
    await explorer.waitFor()
    const firstGroup = explorer.locator('details.resource-group').first()
    const firstGroupSummary = firstGroup.locator(':scope > summary')
    await focusByTab(page, firstGroupSummary, 'first semantic model group')
    await assertElementFocused(page, firstGroupSummary, 'first semantic model group')
    await page.keyboard.press('Enter')
    await expect(firstGroup, 'Enter must expand the focused semantic model group').toHaveAttribute('open', '')
    const firstObject = firstGroup.locator('.object-button').first()
    await focusByTab(page, firstObject, 'first data object')
    await assertElementFocused(page, firstObject, 'first data object')
    const selectionRequest = page.waitForRequest((request) => new URL(request.url()).pathname === '/explore/command' && request.method() === 'POST')
    await page.keyboard.press('Enter')
    await selectionRequest
