...
test('homepage presents the new sections inside the shared site shell', async () => {
  ...
  try {
    ...
    expect(await page.locator('#project-code').textContent()).toContain('# 01 / CONNECT')
    expect(await page.locator('#project-code').textContent()).toContain('# Start with a connection.')
...
test('mobile homepage keeps the shared navigation and all sections usable', async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => document.querySelector('#project-code')?.textContent?.includes('# 06 / PRESENT'))
    ...
    await page.waitForFunction(() => document.querySelector('#project-code')?.textContent?.includes('# 05 / REFRESH'))
