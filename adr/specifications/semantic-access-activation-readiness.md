...
| --- | --- | --- | --- |
| FAI-616 control boundary | In Progress | Production Access requires the PostgreSQL live `ControlStore`; control-boundary ownership is enforced by `TestFAI616LiveControlAuthorityBoundary` in [architecture tests](../../internal/platform/architecture/postgres_access_conformance_test.go). Public role mutation is unexposed; API removal has separate surface tests. | IMPLEMENTED / PARTIAL in ADR-0016; direct FAI-649 blocker with no whole-issue acceptance claim. FAI-617, not FAI-616, owns active runtime projection/restart/reconciliation qualification. |
