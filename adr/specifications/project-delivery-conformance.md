...
| AO-02 | Runtime lineage is lossless where supported; compiler and delivery projections remain authoritative. | FAI-629 extends the export-only [OpenLineage projection](openlineage-conformance.md), reusing existing contract publication, gate, catalog-statistics, and PlanIR evidence authorities. Focused package tests and `internal/platform/architecture/openlineage_conformance_test.go` cover object/facet mapping, one projection path, exact contract imports, and transport/hash isolation. Runtime losslessness remains a separate operational qualification; no runtime publisher or delivery claim is made. | [OpenLineage projection conformance](openlineage-conformance.md); [upgrades](../../docs/articles/operate/upgrades.md) |
