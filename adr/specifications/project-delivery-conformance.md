...
| AO-02 | Supported runtime lineage and quality evidence has an OpenLineage-compatible projection; LeapView ledger remains authority. | FAI-629 extends the export-only [OpenLineage projection](openlineage-conformance.md), reusing existing contract publication, gate, catalog-statistics, and PlanIR evidence authorities. Focused package tests and `internal/platform/architecture/openlineage_conformance_test.go` cover object/facet mapping, one projection path, exact contract imports, and transport/hash isolation. No runtime publisher or delivery claim is made. | [OpenLineage projection conformance](openlineage-conformance.md); [upgrades](../../docs/articles/operate/upgrades.md) |
