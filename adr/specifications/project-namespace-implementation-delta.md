...
| --- | --- |
| Singleton target claim | [`deployment.ProjectClaim`](../../internal/deployment/project_claim.go), the PostgreSQL baseline in [`001_control_plane.sql`](../../internal/platform/postgres/migrations/001_control_plane.sql), and `TestPostgreSQL18BootstrapConcurrentIdentityAndClaimReplay` in [`repository_test.go`](../../internal/platform/bootstrap/postgres/repository_test.go). |
| Atomic claim and candidate start | [`CandidateService.Start`](../../internal/deployment/candidate_service.go) and `TestBootstrapProjectClaimPostgresAuditFailureRollsBackClaim` in [`project_claim_bootstrap_postgres_test.go`](../../internal/deployment/module/project_claim_bootstrap_postgres_test.go). |
...
| --- | --- | --- | --- |
| PRJ-01 | Conforming | The database schema permits only `singleton_id = 1`; `TestPostgreSQL18BootstrapConcurrentIdentityAndClaimReplay` in [`internal/platform/bootstrap/postgres/repository_test.go`](../../internal/platform/bootstrap/postgres/repository_test.go) covers replay, conflict, and concurrency. `TestManagerBindsClaimedProjectBeforeGenerationAndRejectsChanges` in [`internal/runtimehost/project_claim_test.go`](../../internal/runtimehost/project_claim_test.go) rejects a second Project at runtime. | — |
...
| PRJ-04 | Partial | Exact tuple idempotency, conflict, concurrency, and audit rollback pass in [`internal/deployment/module/project_claim_bootstrap_postgres_test.go`](../../internal/deployment/module/project_claim_bootstrap_postgres_test.go). | [FAI-667](https://linear.app/flid/issue/FAI-667) |
| PRJ-05 | Conforming | `TestPostgreSQL18BootstrapConcurrentIdentityAndClaimReplay` preserves the first tuple and rejects replacement; `TestBootstrapImmutableTamperAndCallerRollback` in [`internal/platform/bootstrap/postgres/repository_test.go`](../../internal/platform/bootstrap/postgres/repository_test.go) also rejects direct update and deletion. No retarget API exists. | — |
...
| PRJ-07 | Conforming | Active pointers are Project/environment qualified and admission-fenced; see `TestAdmitGenerationBundleAndActiveRead` in [`internal/servingstate/postgres/repository_test.go`](../../internal/servingstate/postgres/repository_test.go). | — |
| PRJ-08 | Missing | Local authoring now defaults to the Project-free `dashboards/` source root, but local setup still has no durable deployment-authority Project UID state. | [FAI-667](https://linear.app/flid/issue/FAI-667) |
...
| Requirement | Disposition | Current evidence or conflict | Gap owner |
| --- | --- | --- | --- |
| BND-01 | Conforming | [`discoverAuthoredResources`](../../internal/project/compiler/resource_discovery.go) accepts the six analytics kinds from their conventional directories and rejects `kind: Project` plus removed access/publication authoring with migration guidance. [`JSONSchemaFiles`](../../internal/project/schema/schema.go) emits exactly those six public schemas. | — |
| BND-02 | Conforming | [`LoadSourceRoot`](../../internal/project/compiler/source_root.go) recursively discovers the six conventional directories without a root manifest, Project UID, environment, or target input. `TestDiscoverAuthoredResourcesRecursesDeterministically` maintains the contract. | — |
| BND-03 | Conforming | [`artifact.SourceBundle`](../../internal/project/artifact/artifact.go) serializes one target-free graph and resource manifest with no Project identity. `TestSourceBundleIsPortableAndDeterministic` and `TestSourceRootManifestIsCheckoutIndependentAndCanonical` maintain deterministic bytes and digest across checkout roots. | — |
...
| BND-05 | Conforming | [`NewProjectGraph`](../../internal/project/graph/graph.go) rejects control-plane Project nodes, enforces global authored-ID uniqueness, validates closed edges, and rejects cycles. `TestNewProjectGraphIsRootlessAndDeterministic` and `TestNewProjectGraphRejectsControlPlaneProject` maintain the boundary. | — |
...
| API-06 | Implemented | The maintained [`project-cache-idempotency-boundary-audit.md`](project-cache-idempotency-boundary-audit.md) records complete cache and durable replay scopes. The completion adds versioned server target/Project/environment/generation/resource idempotency identity and generation-qualified native refresh scopes while preserving request-digest conflict detection and replay reauthorization. | [FAI-671](https://linear.app/flid/issue/FAI-671) |
| API-07 | Conforming | `TestBoundRuntimeRejectsSecondProject` and `TestProjectAndEnvironmentMismatchRejected` in [`internal/runtimehost/lifecycle_test.go`](../../internal/runtimehost/lifecycle_test.go) reject mixed runtime scope; `TestAdmitGenerationBundleAndActiveRead` in [`internal/servingstate/postgres/repository_test.go`](../../internal/servingstate/postgres/repository_test.go) proves admitted active state remains Project-qualified. | — |
...
| ENV-02 | Conforming | `TestAdmitGenerationBundleAndActiveRead` and `TestServingBundleArtifactCanBeReusedAcrossGenerations` in [`internal/servingstate/postgres/repository_test.go`](../../internal/servingstate/postgres/repository_test.go) bind Project/environment/generation identity and prove immutable admitted publication state. | — |
