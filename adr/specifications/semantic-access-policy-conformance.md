...
| FAI-619 structural authority | The SemanticModel migration to [`api/data-resources/main.tsp`](../../api/data-resources/main.tsp) is complete. Generated [JSON Schema](../../internal/project/contracts/gen/data-resources.schema.json) and [Go DTOs](../../internal/project/contracts/models.gen.go) are the only production structural authority; the [authority guard](../../internal/platform/architecture/architecture_test.go) rejects handwritten CUE/compiler copies. Qualified 2026-09-02 with `task generated:check`, `go test ./internal/platform/architecture -run '^TestSemanticModelStructuralAuthorityIsGeneratedFromTypeSpec$' -count=1`, `go test ./internal/project/contracts ./internal/project/schema -count=1`, and focused `duckdb_arrow` SemanticModel lowering tests. | Implemented |
| STR-01–STR-12 | [Generated-boundary fixtures](../../internal/project/contracts/contracts_test.go), [structural and extracted-YAML fixtures](../../internal/project/schema/semantic_access_contract_test.go), and compiler compatibility/exact-number fixtures cover the migrated structural surface. STR-08 remains partial: standalone transitional `DataPolicy` rejection and contextual access-reference/type-registry resolution are explicitly outside FAI-619 and must not be inferred from the completed TypeSpec migration. | Partial |
| FAI-620 external projection | The generated [`leapview.contract/v1` SemanticModel DTO](../../api/data-resources/contract-projections.tsp) and [allowlist projector](../../internal/project/contractprojection/project.go) include authored grant names, user-attribute names, access-filter conditions, required-grant references, and exact typed values. The reviewed exclusion manifest prevents instance attribute values, AI context, display/discovery prose, and hidden presentation state from entering canonical bytes. Verified with `go test ./internal/project/contractprojection ./internal/project/contracts/generate -count=1` and the cross-language canonical fixture. Standalone `DataPolicy` rejection and contextual registry resolution remain outside this milestone. | Implemented; STR-08 remains Partial |
| ATT-01–ATT-12 | FAI-636 adds the [typed definition contract](../../internal/access/semantic_attributes.go), [PostgreSQL authority](../../internal/access/postgres/semantic_attributes.go), and additive [revision-11 migration](../../internal/platform/postgres/migrations/011_typed_attribute_registry.sql) for stable identity, lifecycle mutation, digest reconciliation, disabled-state behavior, canonical value validation, and transactional audit evidence. Pure and repository qualification uses `go test ./internal/access ./internal/access/postgres ./internal/platform/postgres/migrations ./internal/platform/architecture -count=1` plus the focused race lane. Required live lifecycle and concurrency validation is wired into `task test:go:postgres-conformance` but remains BLOCKED where the Docker socket is unavailable. Assignment, trusted claim mapping, trust-boundary projection, owner-reference validation, optimistic version preconditions, and contextual compiler/runtime resolution remain outside this slice and are not implied by it. | Partial: FAI-636 / ATT-01; live PostgreSQL BLOCKED |
