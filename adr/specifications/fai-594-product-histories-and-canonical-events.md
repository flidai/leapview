...
| --- | --- | --- | --- |
| **Jobs and workflow progress** | LeapView-owned `jobs.job_history`, `jobs.event_sequence`, and append-only `jobs.event` under the Jobs capability ([schema](../../internal/platform/jobs/postgres/schema.sql)); River's `public.river_*` rows are operational execution state. | Product job/resource IDs and per-resource `event_id` are the product identity; event sequence is per resource. The capability route authorizes the resource before history reads. River's numeric job identity, claim, retry, lease, and worker state never become product history. | `jobs.event` remains indefinitely retained. Terminal `jobs.job_history` rows may be removed only through the Jobs-owned bounded maintenance function and its admitted retention policy. River operational-row cleanup, job-claim lease expiry, or worker behavior is not a deletion rule. |
