...
ADR-0017 remains **Implementation: pending**. FAI-637 is the partial
ATT/control-plane slice and implements the access-owned PostgreSQL
control-plane slice: the profile-qualified definition
...
invalidation, or ordinary semantic consumer integration. Candidate/compiler
contextual attribute resolution remains the FAI-639 boundary. The PostgreSQL
repository and contract tests described here are source-level evidence only;
this ADR makes no live PostgreSQL qualification/pass claim.
