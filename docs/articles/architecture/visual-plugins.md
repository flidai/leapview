...
The host action menu provides a keyboard-accessible route to a bounded, formatted
data preview plus copy and CSV export for every non-tabular visual. Preview values
use the shared formatter (`—` for null), report the source row count, and identify
partial or truncated frames instead of implying completeness. Renderer adapters
remain responsible for their own canvas/table semantics, supported zoom or roam
defaults, and preserving an explicitly enabled view state across compatible data
updates. The host announces status, data, and selection changes only when the
