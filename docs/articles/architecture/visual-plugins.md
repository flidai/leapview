...
data preview plus copy and CSV export for inline, non-tabular visuals. Preview
values use the shared formatter (`—` for null), report the source row count, and
identify partial or truncated frames instead of implying completeness. Non-inline
geographic/tiled frames remain renderer-owned and do not expose host data actions
