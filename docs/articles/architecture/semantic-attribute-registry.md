...
The access capability owns the access-owned forward migrations, domain
contracts, direct-pgx repositories, validation, and audit projections.
The generic PostgreSQL migration runner owns only advisory locking, ordered
revision application, checksum evidence, and replay verification. Existing
baseline revisions are immutable; the current branch carries FAI-636 in
platform migration 011 and FAI-637 control state in migration 012.
...
A definition metadata or lifecycle version change makes an older active row
ineligible for effective resolution. The row remains revocable; the next set
atomically tombstones it and creates a new incarnation bound to the current
definition version. Disable followed by restore therefore cannot resurrect a
stale value or mapping.
