...
| Keep | FAI-636 typed registry, migration 011 | `internal/access` owns the typed definition registry: profile-qualified identity, type/shape, stewardship metadata, lifecycle, versioning, deterministic registry digest, and audited mutations. |
| Keep | FAI-637 durable control, migration 012 | `internal/access` owns `semantic_attribute_control_state`, direct principal/group assignments, trusted claim mappings, control digest/revision, tombstone lifecycle, expected-version concurrency, canonical-value validation, and audited mutations. |
...
forward migrations, and existing revisions are never edited.
...
FAI-636 migration 011 and FAI-637 migration 012 follow the same rule. Their
migration IDs and SQL bytes are immutable once qualified and composed in order
after the existing baseline. The registry singleton has
`(profile, registry_revision, registry_digest)` identity. The control singleton
independently has
