...
Target-owned access and publication state applies alongside those layers. The global agent and MCP execute the same governed tools against authorized analytics resources. Separate dev, staging, and production instances can run the same validated source root without requiring a second copy of the YAML tree.
...
A typical analytics source root contains six fixed directories for connections, sources, model tables, semantic models, refresh pipelines, and dashboards. Generated JSON Schemas describe the exact shape of every accepted authored resource; instance control state is managed through authenticated product surfaces.
