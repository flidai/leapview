...
The repository-owned rule file at `deploy/observability/prometheus/leapview-alerts.yaml` provides portable alerts for an unavailable scrape target, sustained application 5xx responses, fatal process-owned DuckDB health, audit-outbox health, recovery qualification freshness, and dashboard refresh error-budget burn. The rules assume that Prometheus assigns LeapView targets the stable scrape label `job="leapview"`; `instance` must identify the bounded scrape target. Alert identity is limited to those scrape labels plus the static `service` and `severity` labels. Operation, publication state, occurrence, schedule, artifact, target, request or trace identities, principals, projects, resources, audit states, and payload identifiers are aggregated away and never become alert labels.
...
Audit-outbox alerts mirror existing readiness boundaries without changing them. `leapview_audit_outbox_scrape_error` reports current aggregate collection failure; its critical alert also detects that metric missing for five minutes from a target whose standard `up` series remains `1`. `leapview_audit_outbox_intents{state=~"poison|quarantined"}` represents unresolved terminal evidence and fires a critical alert after five minutes, with `state` aggregated out of alert identity. The warning backlog alert requires the aggregate undelivered states to remain above 10,000 intents or `leapview_audit_outbox_oldest_undelivered_age_seconds` to remain greater than one hour for ten minutes. Exactly 10,000 intents and exactly one hour remain inside the code-owned readiness boundary. The capacity alert requires a positive configured `leapview_audit_outbox_capacity` together with `leapview_audit_outbox_capacity_remaining <= 0` for five minutes. Aggregate attempt and materialization totals are historical evidence and are deliberately excluded from current-health expressions.

These alerts report the same aggregate failure boundaries used by readiness, but they remain independent signals: an otherwise scrapeable instance can expose an unhealthy outbox without firing the target-unavailable alert. Missing all LeapView scrape data remains the responsibility of `LeapViewTargetUnavailable`; missing only the audit-outbox collection signal on an available target fires `LeapViewAuditOutboxMetricsCollectionFailure`. Prometheus can detect local outbox collection, backlog, terminal-state, and capacity conditions, but it cannot repair the outbox or prove that an external archive or SIEM is complete.

Every alert carries a `runbook_url` for its canonical response procedure. The baseline alerts link directly to [target-unavailable response](/docs/guides/operate/troubleshooting#target-unavailable-alert-fires), [sustained HTTP 5xx response](/docs/guides/operate/troubleshooting#sustained-http-5xx-alert-fires), and [DuckDB fatal-health response](/docs/guides/operate/troubleshooting#duckdb-fatal-health-alert-fires). Audit-outbox, recovery qualification, outcome-reliability burn, and latency burn alerts likewise deep-link to their alert-specific procedures. Use the [correlated incident investigation workflow](/docs/guides/operate/troubleshooting#correlated-incident-investigation-workflow) to move from their bounded alert identity into metrics, request logs, optional upstream trace identity, domain evidence, and an objective recovery record. These links define evidence collection, safe mitigation, and recovery checks; they do not perform remediation. Configure routing and receivers in the operator-owned Alertmanager deployment.
