...
The `leapview:dashboard_refresh_latency:burn_rate_5m` and `leapview:dashboard_refresh_latency:burn_rate_1h` recordings divide the corresponding slow-completed fraction by the 99% SLO's 1% allowance. The critical `LeapViewDashboardRefreshLatencyFastBurn` alert fires only when both burn rates are at least `14.4` for the same `job` and `instance` and the one-hour window contains at least 10 completed refreshes. The dual windows reject a five-minute spike or an older one-hour degradation that does not persist in both windows; the completed-event floor prevents a single slow refresh in sparse traffic from paging. Missing or canceled-only traffic cannot fire the alert. There is no additional hold duration, so recording-rule and alert evaluation can trail the source condition or recovery by one rule-evaluation interval. Follow the linked [latency fast-burn response](/docs/guides/operate/troubleshooting#dashboard-refresh-latency-fast-burn-alert-fires).

Latency and outcome-reliability alerts measure different populations and can fire together: this alert detects slow completed refreshes, while the outcome alerts detect partial, error, or other terminal outcomes among eligible refreshes. Investigate both when they overlap rather than treating either as a duplicate. Alertmanager routing remains operator-owned. This latency SLO defines no percentile recording, slow-burn tier, or automated remediation. It covers completed server-side refresh coordination through terminal target completion, not browser rendering, transport delay, or an end-to-end synthetic journey, and bucket classification cannot expose degradations that remain on the same side of the five-second boundary.
