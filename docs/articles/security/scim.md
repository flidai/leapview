...
SCIM users become ordinary user principals. SCIM groups are global directory groups that may receive access to resources in the server-bound Project. Separate LeapView instances may bind the same directory group independently; the group does not provide Project discovery or switching. Membership changes affect effective access immediately.
...
- creating a group does not grant Project or resource access automatically;
