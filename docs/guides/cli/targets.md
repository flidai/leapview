...
LeapView reads the target's immutable instance identity and environment before synchronizing the source root. The local candidate handoff is keyed by both the source root and the target origin, so `publish` cannot silently promote a candidate created on another target.
...
source root and target unchanged between review and publication. Target-owned
