...
and host marker while leaving the private application environment untouched.
It never reads, logs, uploads, or duplicates the provider key. Any failed write, restart,
...
legacy `stage`, `prepare`, and `deploy` jobs target the old installation and
are no longer selectable. The replacement host uses only the exact,
revision-pinned `compose-deploy` action described above.
...
The legacy `scripts/rollout_demo_runtime.sh` path receives `DEEPSEEK_API_KEY`
from the protected Infisical `prod:/demo/deployment` path and writes the mapped
provider variables to its private mode-0600 `runtime.env`. The Compose replacement host owns its
deployment-managed provider configuration in the private mode-0600
`leapview.env`. A `compose-deploy` rollout verifies that
`LEAPVIEW_AGENT_API_KEY`, `LEAPVIEW_AGENT_BASE_URL`, and
`LEAPVIEW_AGENT_MODEL` are configured, but deliberately leaves their values
untouched and never copies them into the runner, command line, logs, or rollback
artifacts. The provider key is never committed. Provider configuration enables
the runtime while access policy continues to control which resources its tools
can reach.
