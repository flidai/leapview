...
for an existing-host release transition. A caller driving the transition
runner invokes it once per effect; the runner retains ownership of the durable
operation, target fence, and phase records. Run it from a controller build
...
After the runner records staging, invoke the same command with
...
returns a JSON effect result for the runner to validate and persist before
moving to the next phase. An operator
should not advance phases independently of the runner.
...
generation. Only after the runner records staging does activation switch
...
image. The runner records each phase result in the existing transition
