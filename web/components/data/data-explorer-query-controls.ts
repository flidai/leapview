  ...
  private renderConfig(spec: ExplorationSpec) {
    ...
    return html`<details class="query-config" aria-label="Query configuration" open>
      ...
      <div class="config-grid">
        <label>Time field<select aria-label="Time field" .value=${spec.time?.field ?? ''} @change=${(event: Event) => this.changeTimeField((event.target as HTMLSelectElement).value, spec)}><option value="" .selected=${!spec.time?.field}>No time field</option>${timeFields.map((field) => html`<option value=${field.id} .selected=${field.id === spec.time?.field}>${field.label || field.id}</option>`)}</select></label>
