...
test('model entry uses the authoritative bare resource ID from the bootstrap', () => {
  const href = modelExploreHref({ id: 'orders', key: 'model:orders', title: 'Orders', type: 'model', typeLabel: 'Model' } as ResourceAssetSummarySignal)
  expect(href).toBe('/explore?object=orders&returnSurface=model&returnAsset=orders&returnSection=data')
})
...
test('non-model assets have no model explorer entry', () => {
  ...
  expect(modelExploreHref({ id: 'semantic:sales', key: 'sales', title: 'Sales', type: 'semantic_model', typeLabel: 'Semantic model' } as ResourceAssetSummarySignal)).toBeUndefined()
})
