...
export function builderTestDocument(options: { agentReady?: boolean } = {}): string {
  const visualCatalog = [
  ...
  const signals = {
    ...
    ...(options.agentReady ? {
      agent: { transcript: [], status: { enabled: true, running: false }, composer: { value: '', disabled: false } },
      agentContext: { pageTitle: 'Overview' },
    } : {}),
  }
