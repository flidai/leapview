...
class LeapViewDashboardBuilder extends DatastarLit(LitElement) {
  ...
  private renderToolbar(builder: DashboardBuilderSignal) {
    ...
    const canArchive = builder.capabilities.canArchive && !canDelete
    const hasMoreActions = builder.capabilities.canShare || builder.capabilities.canExport || canArchive || canDelete || Boolean(this.forkHref)
            ...
            <details class="more-actions">
              ...
              <div class="more-menu" aria-label="More dashboard actions">
                ...
                ${builder.capabilities.canExport
                  ...
                  : nothing}
                ${canArchive ? html`<button type="button" class="archive-action" data-builder-action="archive" @click=${this.archiveDashboard}>${lucideIcon(Archive, { size: 14, strokeWidth: 2 })}<span>Archive dashboard</span></button>` : nothing}
