...
test('dashboard agent drawer carries page context and explicit visual references', async () => {
  ...
  try {
    ...
    const visualActionsAtRest = await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      ...
      const reportTable = (table.shadowRoot as ShadowRoot).querySelector('lv-report-table') as any
      await reportTable.updateComplete
      const tableExpand = (reportTable.shadowRoot as ShadowRoot).querySelector('.visual-actions .icon-action') as HTMLElement
      const tableOptions = (reportTable.shadowRoot as ShadowRoot).querySelector('.visual-options summary') as HTMLElement
      const askStyle = getComputedStyle(ask)
      const expand = (chart.shadowRoot as ShadowRoot).querySelector('[data-visualization-expand]') as HTMLElement
      ...
      return {
        ...
        askUsesAgentIcon: ask.querySelector('svg')?.innerHTML === agentIconMarkup
          && (drawer.shadowRoot as ShadowRoot).querySelector('.title svg')?.innerHTML === agentIconMarkup,
        chartAction: expand.getAttribute('aria-label'),
        tableHasExpand: Boolean((table.shadowRoot as ShadowRoot).querySelector('[data-visualization-expand]')),
      }
    })
    ...
    const visualActionsOnHover = await page.locator('lv-dashboard-page').evaluate((element: any) => {
      const frame = (element.shadowRoot as ShadowRoot).querySelector('[data-visual-id="orders_chart"]') as any
      const chart = frame.querySelector('lv-visualization-host') as any
      const ask = chart.querySelector('.ask-visual') as HTMLElement
      const expand = (chart.shadowRoot as ShadowRoot).querySelector('[data-visualization-expand]') as HTMLElement
      const askStyle = getComputedStyle(ask)
    ...
    await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      (element.shadowRoot as ShadowRoot).querySelector<HTMLButtonElement>('.agent-toggle')!.click()
      await element.updateComplete
      const drawer = (element.shadowRoot as ShadowRoot).querySelector('lv-chat-drawer') as TestDomElement
    ...
    const opened = await page.locator('lv-dashboard-page').evaluate((element: any) => {
      const root = (element.shadowRoot as ShadowRoot)
      const drawer = root.querySelector('lv-chat-drawer') as any
      const drawerRoot = (drawer.shadowRoot as ShadowRoot)
      ...
      return {
        ...
        threadSharesSurface: getComputedStyle((thread.shadowRoot as ShadowRoot).querySelector<HTMLElement>('.thread')!).backgroundColor === getComputedStyle(drawerSurface).backgroundColor,
        composerDockBorder: getComputedStyle(composer).borderTopStyle,
        composerShadow: getComputedStyle((composer.shadowRoot as ShadowRoot).querySelector<HTMLElement>('.composer-surface')!).boxShadow,
        composerHeight: Math.round((composer.shadowRoot as ShadowRoot).querySelector<HTMLElement>('.composer-surface')!.getBoundingClientRect().height),
    ...
    const groupedSearch = await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      await element.updateComplete
      const drawer = (element.shadowRoot as ShadowRoot).querySelector('lv-chat-drawer') as any
      await drawer.updateComplete
      const composer = (drawer.shadowRoot as ShadowRoot).querySelector('lv-chat-composer') as any
      const textarea = (composer.shadowRoot as ShadowRoot).querySelector('textarea') as HTMLTextAreaElement
      textarea.value = '@orders'
      textarea.setSelectionRange(textarea.value.length, textarea.value.length)
      textarea.dispatchEvent(new InputEvent('input', { bubbles: true, composed: true }))
      await composer.updateComplete
      return {
        labels: Array.from((composer.shadowRoot as ShadowRoot).querySelectorAll('.mention-section-label')).map((node) => node.textContent.trim()),
        options: Array.from((composer.shadowRoot as ShadowRoot).querySelectorAll('.mention-option')).map((node) => node.textContent.replace(/\s+/g, ' ').trim()),
        onPage: Array.from((composer.shadowRoot as ShadowRoot).querySelector('[aria-label="On this page"]')?.querySelectorAll('.mention-option') ?? []).map((node) => node.textContent.replace(/\s+/g, ' ').trim()),
        accessible: Array.from((composer.shadowRoot as ShadowRoot).querySelector('[aria-label="All accessible"]')?.querySelectorAll('.mention-option') ?? []).map((node) => node.textContent.replace(/\s+/g, ' ').trim()),
      }
    })
    ...
    await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      const frame = Array.from((element.shadowRoot as ShadowRoot).querySelectorAll('lv-dashboard-visual-frame'))
    ...
    const referenced = await page.locator('lv-dashboard-page').evaluate((element: any) => {
      ...
      return {
        ...
        highlighted: Boolean((element.shadowRoot as ShadowRoot).querySelector('lv-dashboard-visual-frame[data-agent-referenced]')),
		pressed: (element.shadowRoot as ShadowRoot).querySelector('[data-visual-id="orders_chart"] .ask-visual')?.getAttribute('aria-pressed'),
      }
    })
    ...
    const limitReached = await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      ...
      await composer.updateComplete
      return {
        chips: Array.from((composer.shadowRoot as ShadowRoot).querySelectorAll('.reference-chip')).map((node) => node.textContent?.replace(/\s+/g, ' ').trim()),
        status: (drawer.shadowRoot as ShadowRoot).querySelector('[data-reference-limit-status]')?.textContent?.replace(/\s+/g, ' ').trim(),
      }
    })
    ...
    const submitted = await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
      ...
      ;((composer.shadowRoot as ShadowRoot).querySelector('form') as HTMLFormElement).dispatchEvent(new Event('submit', { bubbles: true, cancelable: true }))
	...
	const accepted = await page.locator('lv-dashboard-page').evaluate(async (element: any) => {
	  ...
	  await drawer.updateComplete
	  const composer = (drawer.shadowRoot as ShadowRoot).querySelector('lv-chat-composer') as any
	  const thread = (drawer.shadowRoot as ShadowRoot).querySelector('lv-chat-thread') as any
	  await Promise.all([composer.updateComplete, thread.updateComplete])
	  return {
		composerReferences: composer.references.length,
            draft: (composer.shadowRoot as ShadowRoot).querySelector<HTMLTextAreaElement>('textarea')!.value,
		bubble: (thread.shadowRoot as ShadowRoot).querySelector('.message.user .bubble')?.textContent?.replace(/\s+/g, ' ').trim(),
		highlighted: Boolean((element.shadowRoot as ShadowRoot).querySelector('lv-dashboard-visual-frame[data-agent-referenced]')),
	  }
	})
...
test('dashboard agent drawer folds out with the dashboard motion contract', async () => {
  ...
  try {
    ...
    const reducedMotionDuration = await page.locator('lv-dashboard-page').evaluate((element: any) => (
      getComputedStyle((element.shadowRoot as ShadowRoot).querySelector<HTMLElement>('.route')!).transitionDuration
    ))
...
test('dashboard agent restores its open state and active conversation after reload', async () => {
  ...
  try {
    ...
    const closedState = await page.locator('lv-dashboard-page').evaluate((element: any) => ({
      open: ((element.shadowRoot as ShadowRoot).querySelector('lv-chat-drawer') as (TestDomElement & { open?: boolean }) | null)?.open,
