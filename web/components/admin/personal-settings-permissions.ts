...
export function tokenPermissionPolicies(capabilities: PersonalCapabilityOptionSignal[]): TokenPermissionPolicy[] {
  ...
  capabilities.forEach((capability, index) => {
    ...
    const option: TokenPermissionOption = { id: `typed-${index}`, label: capability.label, permissions }
    const pair = permissions[0]
    if (permissions.length !== 1 || !pair) {
      policies.set(`bundle-${index}`, {
        id: `bundle-${index}`, label: capability.label, description: capability.description, category: capability.category,
    ...
    const key = [pair.profile, pair.action, targetScope, target.projectId ?? '', target.instanceId ?? '', resourceKind].join('|')
