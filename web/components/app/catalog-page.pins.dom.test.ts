...
test('eight dashboard copies keep favorites and pins independent across catalog views', async () => {
  ...
  try {
    ...
    await pinned.getByRole('button', { name: 'Unpin Sales copy 1' }).click()
    ...
    await catalog.getByRole('table', { name: 'My dashboards' }).getByRole('button', { name: 'Pin Sales copy 1' }).click()
    expect(await pinned.getByRole('link').count()).toBe(8)
